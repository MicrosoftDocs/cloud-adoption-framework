...
1. ***Define your cloud operations model.*** Choose a centralized or shared management model based on your organization's size and maturity, outlined in the table below:
...
1. ***Define deployment procedures (release management).*** Standardize your deployments, releases, and environment promotions to maintain consistent configurations.  See [Manage deployments](./administer.md#manage-cloud-resources).

1. ***Define disaster recovery and business continuity procedures.*** Prepare a standardized response plan to handle potential failures. See [Manage disaster recovery and business continuity](./protect.md#manage-business-continuity).
...
1. ***Create a library of Azure-centric runbooks.*** Create Azure-specific runbooks addressing scenarios such as high CPU usage [Azure App Service](/azure/app-service/manage-scale-up) failover and failback with [Azure Site Recovery](/azure/site-recovery/failover-failback-overview-modernized), blue/green deployments with [Azure Front Door](/azure/frontdoor/blue-green-deployment), or backup restoration in [Azure Blob Storage](/azure/storage/common/storage-disaster-recovery-guidancezure) and [Azure Cosmos DB](/azure/cosmos-db/restore-account-continuous-backup).
...
1. ***Implement operations programmatically.*** Integrate infrastructure-as-code into your runbooks to provision common resources consistently and accurately each time.
...
| Automation        | Teams reuse IaC templates, automation scripts, and best practices across projects.                                 |
...
Improving operations involves optimizing your Azure cloud environment. Invest in continuous learning and actively seek feedback. Follow these steps:

1. ***Review operations to improve.*** Follow best practices to [monitor](./monitor.md#configure-monitoring) the health, compliance, security, costs, data, and cloud resources. Hold weekly operational review meetings focused on key metrics, recent incidents, implemented changes, and upcoming risks. Address [resource sprawl](./administer.md#manage-resource-sprawl) and [technical debt](/azure/well-architected/performance-efficiency/continuous-performance-optimize#address-technical-debt). This structured review practice helps your teams continuously refine processes and ensures procedures remain effective.

1. ***Train for operations.*** Promote continuous learning by bookmarking essential operational resources. Ensure your cloud operations remain dynamic and current by providing sandbox environments for practical training.
