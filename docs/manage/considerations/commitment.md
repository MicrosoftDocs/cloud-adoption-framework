...
When your organization establishes a commitment, determine how to align the following aspects.
...
Your organization might also [outsource some of your ongoing management functions to a service provider](https://azure.microsoft.com/partners). Service providers can use [Azure Lighthouse](/azure/lighthouse/overview) to provide your organization with precise control. For example, you can grant access to your resources and have greater visibility into the actions that service providers perform.
...
- **Centralized responsibility model:** Your organization might require a *central IT team model* if you have compliance requirements, technical complexity, or some shared service models. In a central IT model, IT performs its operations management responsibilities.
...
- **Mixed model:** Classification is the foundation of a *mixed model* of management responsibilities. If your organization is in the process of transforming from on-premises to the cloud, you might require an on-premises-first operating model for some time. If your organization has strict compliance requirements or depends on long-term contracts with IT outsourcing vendors, you might need a centralized operating model.
...
**Which organization is responsible for day-to-day operations management for this workload?**
...
The next commitment that your organization should make is to accept the costs that are associated with the baseline-managed offering.

**Does your organization agree to invest in the baseline offering to meet the minimum standards of cloud operations?**

If your organization doesn't agree to that level of management, you must create a solution so that your organization can proceed. Ensure that your solution doesn't materially affect the cloud operations of other workloads.
...
For managed solutions, you can apply several design principles and template solutions in addition to the management baseline. Each design principle for reliability and resiliency adds operating cost to the workload. IT and your organization must agree on these extra commitments, so you must understand potential losses that you can avoid when you implement more principles.
...
The composite service-level objective (SLO) is the SLA that's based on the deployment of each asset in the workload. The composite SLO field drives the *estimated outage* (labeled `Est.Outage` in the workbook). To calculate the estimated outage in hours per year without using the workbook, apply the following formula:
...
The *standard loss impact* (labeled `Standard Impact` in the workbook) forecasts the financial impact of any outage, assuming that the *estimated outage* prediction proves accurate. To calculate this forecast without using the workbook, apply the following formula:

> *Standard impact = estimated outage @ three 9s of uptime &#215; time-value impact*

The value serves as a baseline for cost, should the business stakeholders choose to invest in a higher level of management.

#### Composite-SLO impact

The *composite-SLO impact* (labeled `Commitment level impact` in the workbook) provides the updated fiscal impact, based on the changes to the uptime SLA. Use this calculation to compare the projected financial impact of both options. To calculate this forecast impact without the spreadsheet, apply the following formula:
...
The *comparison basis* evaluates standard impact and composite SLO impact to determine which is most appropriate in the return column.
...
At this point in the process, commitments have been made: centralized or delegated responsibility, Azure tenancy, and level of commitment. You have validated and documented each commitment to ensure that the cloud operations team, cloud strategy team, and business stakeholders align on this commitment to manage the workload.

## Next steps

After you make commitments, the responsible operations teams can configure the workload in question. To get started, evaluate various approaches to [inventory and visibility](./inventory.md).
