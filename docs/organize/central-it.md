...
**Isolation:** Since the business can provide its own ongoing operations management, and since the solution isn't dependent on direct traffic to other internal assets, it can be cordoned off in a new subscription. That subscription is also added to a separate node of the new management group hierarchy.
