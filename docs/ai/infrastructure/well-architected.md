...
- *Pick the right deployment options.* Use the [CycleCloud Workspace for Slurm](/azure/cloud-adoption-framework/ai/infrastructure/ai-azure-infrastructure-implementation) to simplify deployment. Choose NC-series or ND-series SKUs for the GPU partition. For multinode training, ND-series SKUs are recommended for RDMA support. Azure's HPC marketplace images generally include these drivers and libraries. If you need customization, the azhpc-images repository can ensure compatibility.
...
Both of these components are included in [CycleCloud Workspace for Slurm](/azure/cloud-adoption-framework/ai/infrastructure/ai-azure-infrastructure-implementation) but aren't currently included in Slurm clusters that are built via CycleCloud. You can introduce these extra components via [cluster-init with CycleCloud projects](/azure/cyclecloud/how-to/projects). When you meet these requirements, you can use Megatron-LM for LLM training by:
