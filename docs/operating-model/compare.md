...
- **Guidance**: Decentralized operations are limited to workload level decisions. The Microsoft Azure Well-Architected Framework is designed to support the decisions made within that scope. The processes and guidance within the Cloud Adoption Framework are likely to add overhead that aren't required by decentralized operations.
...
- **Standardization**: Use source code and deployment pipelines to standardize the environment from release to release.
...
- **Guidance**: Azure landing zone implementation options that align to the "start small and expand" development velocity create a good starting point. Those options can accelerate adoption efforts. To be successful, establish clear policies to guide early adoption efforts within acceptable risk tolerances. Govern and Manage methodologies create processes to mature operations in parallel. Those steps serve as stage gates that must be completed before allowing for increased risk as operations matures.
...
- **Landing zone design**: Landing zone design takes a dependency on the foundational utilities. Each workload-focused team must understand what's included in the design and what isn't allowed. This design helps avoid duplication of effort or conflicts with automated governance. Factor exception processes in to landing zone designs to create flexibility.
- **Foundational utilities**: Foundational utilities can be used to develop a solution that scales to meet various adoption plans. Choosing how to centralize the foundational utilities can delay early adoption efforts. But, the delay should be offset later in the process by accelerations and blocker avoidance.
...
- **Guidance**: Start with a thorough review of the portfolio using the approach that's outlined in the [business alignment](../manage/considerations/business-alignment.md) articles. Take care to group the portfolio by state operating model (decentralized, centralized, or enterprise).
