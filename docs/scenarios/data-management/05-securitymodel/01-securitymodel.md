...
Enterprise Analytic and AI relies on a number of security features which can be enabled on Azure. They include Encryption, Role Based Access Control, Access Control Lists and Networking restrictions.

## Data-at-Rest Encryption

Data-at-Rest (DaR) encryption refers to the encryption of data as it persists in storage. DaR encryption is a critical security control by Microsoft as it addresses security risk related to direct physical access to storage media as the underlying data is not recoverable and cannot be changed without the underlying decryption key. This makes it an important layer in the defence-in-depth strategy of Microsoft data centres. In addition, there are often compliance and governance reasons to deploy DaR encryption.
...
Where Advanced Threat Protection is available for the PaaS Services then we would recommend enabling this feature by default.
...
It is important to understand how Enterprise Scale Analytics and AI use Active Directory identities to secure data in the data lake accounts. This section gives a the core terms required for the solution pattern but for a depth explanation see [Access control lists (ACLs) in Azure Data Lake Storage Gen2](https://docs.microsoft.com/azure/storage/blobs/data-lake-storage-access-control).
...
|Limits|2000 RBAC in a subscription.|32 ACLs (effectively 28 ACLs) per file, 32 ACLs (effectively 28 ACLs) per folder, default, and access ACLs each.|
...
#### Example Scenario for using Azure AD Security Groups with non-sensitive data
...
To enable these activities, the Domain could create a `LogsWriter` group and a `LogsReader` group. Then, you could assign permissions as follows:
...
Other polygot storage such as PostgreSQL, MySQL, Azure SQL Database, SQL Managed Instance, and Azure Synapse Analytics are likely to be used in the Enterprise Scale Analytic solution pattern. They could be used by Domains to store there Read Data Stores or by Data Product Teams.
