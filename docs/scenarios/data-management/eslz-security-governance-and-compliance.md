...
\
If the dataset is *confidential or below*, Users Principal Names(UPNs) and Service Principal object can be added to these groups. If the dataset is *sensitive (PII)*, we would recommend that only Service Principal object and/or Managed Service Identity (MSI) be added to these groups. Please see [Data Privacy](data-privacy.md) for further details on our *sensitive (PII)* data security pattern.
