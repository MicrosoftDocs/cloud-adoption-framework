...
Consider assigning the _Reader_ role to the Azure Purview managed identity in each data landing zone subscription before you register any of these data sources into Azure Purview:
...
- Scale up when the processor and memory of the node are under-utilized
...
You might require to set up a _Credential_ using a Key Vault _Secret_ for specific scenarios:
...
|Data Analysts and Performance Analysts |Use Azure to discover and share new insights from existing data assets or *ad hoc* data. Create one-click AI transformations, consume pre-built models, and easily generate machine learning models.|Purview Data Source Administrator and Data Reader   |
...
| ML Engineers | Enable right processes and infrastructure for easy model deployment and model management. |Purview Data Source Administrator and Data Curator    |
