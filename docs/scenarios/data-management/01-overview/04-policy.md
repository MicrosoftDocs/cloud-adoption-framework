...
Before considering a deployment it is important for the business to decide up on the guard rails that you wish to put in place.

## Azure Policies for Analytics and AI

A core principle of the Enterprise Scale Analytics and AI solution pattern is to move away from a centralized IT lead solution and to enable agility by making it easy to create, read, update, and delete resources as needed. However, while giving unrestricted resource access to developers can make them very agile, it can also lead to unintended cost consequences. The solution to this problem is resource access governance; the ongoing process of managing, monitoring, and auditing the use of Azure resources to meet the goals and requirements of your organization. [Enterprise-Scale Architecture](https://docs.microsoft.com/azure/cloud-adoption-framework/ready/enterprise-scale/) already uses this and Enterprise Scale Analytics and AI solution pattern builds upon these standards by adding additional Azure Policies which are applied to our Data Management Subscription and Data Landing Zones.
...
Azure Policy should be used to implement governance and enforce rules for resource consistency, regulatory compliance, security, cost, and management. Prebuilt policies are available and should be used to save time.
...
Cost transparency across a technical estate is a critical management challenge faced by every large enterprise organization. Different principals should be followed to reduce, optimize, and allocate cost between organizations.
...
You should use Azure Policy to set and track ARM tagging policies. For example, we could have:
...
|Azure Purview| linkedServices (deployIfNotExists) |Automatically add all data sources in Data Landing Zones to the data catalogue in the Data Management Subscription, so that crawlers can detect and index datasets.|
