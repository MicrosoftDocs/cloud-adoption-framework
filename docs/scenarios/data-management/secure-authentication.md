...
In the cloud, Azure Active Directory (Azure AD) is a centralized identify provider and the preferred source for identity management. Delegating authentication and authorization to Azure AD enables scenarios such as conditional access policies that require a user to be in a specific location. It supports multifactor authentication to increase the level of access security. Configure data lake data store services with Azure AD integration when possible.
...
- Managed identity credentials are maintained, managed, and stored in Azure AD. There's no password for user to keep.
...
|-------------------|------------------|
| A security identity manually created in Azure AD for use by application, services, and tools to access specific Azure Resources. | A managed identity is a special type of service principal. It's an automatic identity that created when an Azure service is created. |
...
| This identity has an independent lifecycle. You must delete it explicitly. | Deletes automatically when the Azure service instance is deleted. |
