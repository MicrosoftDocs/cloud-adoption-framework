...
|Sensitive (personal data) |Personal data that must be masked and shared only on a need to know basis for a limited time Cannot be sent to unauthorized personnel or outside the company |
...
The tables can be made available to Azure Databricks with [Apache Spark connector: SQL Server and Azure SQL](/sql/connect/spark/connector).
