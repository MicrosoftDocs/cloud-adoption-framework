...
1. **Integrate Azure Key Vault for Azure-native key management with private connectivity.** Azure Key Vault provides centralized TDE master encryption key storage within Azure security boundaries while maintaining compliance with Azure governance policies. Implementation requirements differ by database technology:
   - **Oracle Exadata Database@Azure** requires Private Endpoint connectivity, advanced networking features, and OCI Private DNS zone configuration. For complete implementation guidance including network setup, identity configuration, and validation procedures, see [Azure Key Vault integration for Oracle Exadata Database@Azure](oracle-azure-key-vault-integration-exadata.md).
