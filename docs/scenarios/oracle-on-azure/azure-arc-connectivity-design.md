...
4. **Plan for bandwidth requirements across regions**. Cross-region Arc connectivity introduces additional bandwidth costs and latency considerations for heartbeat traffic, monitoring data, and policy evaluation communications. For bandwidth planning, see [Azure bandwidth pricing](https://azure.microsoft.com/pricing/details/bandwidth/).
...
2. **Exclude specific Azure Arc extensions from TLS inspection when required**. Some Azure Arc extensions require exclusion from TLS inspection to function properly. Identify these extensions and configure appropriate bypass rules. For extension management guidance, see [Azure Arc-enabled servers extensions](/azure/azure-arc/servers/manage-vm-extensions).

3. **Maintain security monitoring for Arc traffic**. Configure your TLS inspection infrastructure to monitor and log Arc communications while allowing the necessary traffic to flow. This approach provides security visibility without blocking functionality. For security monitoring patterns, see [Azure Monitor network insights](/azure/azure-monitor/insights/network-insights-overview).

4. **Test Arc functionality after TLS inspection configuration**. Validate that all Arc features work correctly with your TLS inspection setup before deploying to production environments. For testing and validation guidance, see [Troubleshoot Azure Arc-enabled servers connectivity](/azure/azure-arc/servers/troubleshoot-connectivity).
...
2. **Configure security endpoint accessibility**. Verify that [Microsoft Defender for Cloud network requirements](/azure/defender-for-cloud/defender-for-cloud-introduction) and [Microsoft Sentinel integration endpoints](/azure/sentinel/connect-services-diagnostic-setting-based) are accessible through your Arc connectivity setup.

3. **Coordinate with existing Oracle security monitoring**. Integrate Defender alerts with existing Oracle security tools to avoid duplication while enhancing overall security posture. For integration patterns, see [Microsoft Defender for Cloud integration](/azure/defender-for-cloud/integration-overview).
...
4. **Route Arc traffic through existing security monitoring tools**. Ensure that Arc communications flow through your established network security monitoring infrastructure to maintain visibility and compliance with security policies.
...
2. **Implement network connectivity diagnostics**. Use [Arc connectivity troubleshooting tools](/azure/azure-arc/servers/troubleshoot-connectivity) to diagnose network path issues and validate endpoint accessibility.

3. **Integrate with existing Oracle monitoring systems**. Coordinate Arc connectivity monitoring with Oracle Enterprise Manager alerts and include Arc endpoint health in existing network monitoring dashboards.
...
2. **Record network path requirements and bandwidth utilization**. Document the network paths used for Arc connectivity and expected bandwidth utilization patterns. This information supports capacity planning and performance troubleshooting.

3. **Map integration points with existing network security controls**. Document how Arc connectivity integrates with your existing network security infrastructure. This mapping supports security reviews and incident response procedures.

4. **Maintain troubleshooting procedures for Arc connectivity issues**. Create step-by-step procedures for diagnosing and resolving common Arc connectivity problems. Include contact information and escalation procedures for complex issues.
...
2. **Implement machine configuration policies for operating system-level network controls**. Use [Azure machine configuration policies](/azure/governance/machine-configuration/overview) to enforce network security settings at the operating system level. This approach provides GPO-like control over Arc-connected servers and ensures consistent security configurations.

3. **Establish governance baseline for comprehensive security management**. Follow the [Governance, security, and compliance baseline for Azure Arc-enabled servers](/azure/cloud-adoption-framework/scenarios/hybrid/arc-enabled-servers/eslz-security-governance-and-compliance) to implement comprehensive governance across your hybrid infrastructure.

4. **Monitor policy compliance and remediation status**. Regularly review policy compliance reports and implement automated remediation where possible to maintain security standards across your Arc-enabled infrastructure.
...
4. **Prepare configuration templates and automation**. Create standardized configuration templates for Arc deployment across your Oracle infrastructure. Prepare automation scripts for consistent deployment and configuration management.
