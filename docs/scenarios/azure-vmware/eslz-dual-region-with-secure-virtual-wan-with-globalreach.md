...
Going into more detail, the Azure VMware Solution private cloud's preferred default route "∞ 0.0.0.0/0" is received via connection "D" from its local regional hub. Additionally, the Azure VMware Solution private cloud receives a backup default route "△ 0.0.0.0/0," which originates on the cross-regional hub and advertised across the Global Reach (C) connection. This cross-regional backup default route "△ 0.0.0.0/0" is also advertised over Global Reach (A) and Global Reach (B). However, Global Reach (C) is the primary choice for the cross-regional backup default route "△ 0.0.0.0/0" because it is only one hop away from the cross-regional hub, making it the most attractive option. Global Reach (A) and Global Reach (B), being two hops away from the cross-regional hub, are considered less preferred. In the event of a local regional outage, if Global Reach (C) is not configured or is down, then cross-regional backup internet traffic will be rerouted through Global Reach (A) and Global Reach (B). Ensure that you explicitly configure Global Reach (A), Global Reach (B), and Global Reach (C). It is imperative to do this to prevent potential suboptimal routing between Global Reach sites. 
