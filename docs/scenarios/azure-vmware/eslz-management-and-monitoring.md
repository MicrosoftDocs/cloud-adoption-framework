...
1. Azure VMware Solution implements a default storage policy with thick provisioned enabled. For efficient use of vSAM capacity, evaluate creating and using a vSAN storage policy that implements [thin provisioned virtual hard disks](https://docs.vmware.com/en/VMware-vSphere/6.7/com.vmware.vsphere.virtualsan.doc/GUID-08911FD3-2462-4C1C-AE81-0D4DBC8F7990.html) for workload virtual machines. This configuration allows for a more efficient use of vSAN capacity. The risk factor is low if customers adopt the monitoring methodology above for vSAN datastore capacity alerting.
1. Another consideration is deciding upon thick or thin provisioned disks. Each virtual machine's disk configuration may vary. Additionally, individual disks on the virtual machine may vary. A virtual machine could have thick or thin disks, dependent upon performance requirements of the workload. Similarly, the virtual machine could hav both thick and thin disks configured. These two extra components make evaluating the storage policy a firm requirement to ensure the vSAN storage is never over capacity.
