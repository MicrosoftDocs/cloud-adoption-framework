...
- vSAN storage is a finite resource that needs to be managed to maintain availability and performance. Familiarize yourself with the Azure VMware Solution storage concepts documented in the [Azure VMware Solution storage concepts page](/azure/azure-vmware/concepts-storage). Use vSAN storage for guest virtual machine (VM) workloads only. Review the following design considerations to reduce unnecessary storage use on vSAN.
...
- [Configure the following alerts in Azure Monitor](/azure/azure-monitor/alerts/alerts-overview) for when the Azure VMware Solution cluster is nearing key threshhold values for disk, CPU, or RAM:
