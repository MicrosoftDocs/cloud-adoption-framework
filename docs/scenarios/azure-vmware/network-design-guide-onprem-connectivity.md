...
- High throughput: ExpressRoute allows connecting to the edge of the Microsoft network from your premises over dedicated lines, and to provision the network capacity you need (up to 10 Gbps for provider-based ExpressRoute, or 100 Gbps for ExpressRoute Direct). 
...
- In a customer-managed virtual network, it is possible to customize routing so that traffic between Azure VMware Solution private clouds and on-premises sites is inspected by firewall devices deployed in an Azure virtual network.
