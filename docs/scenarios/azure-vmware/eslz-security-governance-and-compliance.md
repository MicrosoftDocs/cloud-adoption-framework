...
This article describes how to implement and holistically govern a secure Azure VMware Solution throughout its life cycle. The article explores specific design elements and provides targeted recommendations for Azure VMware Solution security, governance, and compliance design.
...
- **Inbound internet request logging for guest workloads.** Implement firewall audit reporting on HTTP(s) and non-HTTP(s) endpoints to comply with regulatory requirements like Sarbanes-Oxley (SOX) and Payment Card Industry (PCI) standards.
...
- **Guest VM security posture governance.** Security compliance awareness for Azure VMware Solution guest VMs helps you understand cybersecurity readiness and response and provide complete security coverage for Azure VMware Solution guest VMs and applications.
...
- **Logging and monitoring governance**. Enable diagnostics metrics and logging on guest VMs to more easily debug guest and application issues. Implement near real-time insights for prompt detection of performance bottlenecks and operational issues.
...
- **Update governance.** Delayed or incomplete updates or patching are top attack vectors that can result in exposing or compromising Azure VMware Solution guest VMs and applications. Ensure timely update installations on guest VMs.
