...
The Enterprise Scale Landing Zone sets forward a series of best practices for Automation & DevOps which can assist with the deployment of an Azure VMware Solution (AVS) Private Cloud. This solution guide provides an overview of deployment considerations for both the initial deployment of AVS and guidance for operational automation. This implementation follows the architecture and best practices of the Cloud Adoption Framework, focusing on designing for scale. 

This solution is made up of two key parts; guidance around deployment & automation practices for the Azure VMware Solution and a set of open-source artifacts that can be adapted to assist in the deployment of your Private Cloud. While this solution aims to set forward an end-to-end automation journey, it is open to the implementing organization to decide what components are manually deployed based on the considerations below. 
...
## Deployment strategy
### Manual deployment, connectivity & scale
...
- When deploying AVS via the Azure Portal, [Azure CLI](/cli/azure/vmware/private-cloud?view=azure-cli-latest#az_vmware_private_cloud_create), or [Azure Powershell Modules](/powershell/module/az.vmware/new-azvmwareprivatecloud?view=azps-6.1.0), a series of terms and conditions relating to data protection within the solution is presented. If you are using the ARM API’s directly or deploying via an ARM or Bicep template, consider reviewing these [terms and conditions](https://azuremarketplace.microsoft.com/en-us/marketplace/apps/Microsoft.AVS?tab=Overview) before deploying automation.
...
- Consider using the deployments blade within the target resource group to monitor the private cloud creation process. Once the private cloud has been deployed, confirm that it is in the “Succeeded” status before proceeding. If the private cloud shows [a “Failed” status](/azure/azure-vmware/fix-deployment-provisioning-failures), you may be unable to connect to vCenter, and removal & redeployment of the Private Cloud may be required.
...
#### Recommendations:
- If utilizing separate service principals for AVS deployment vs. ExpressRoute configuration Key Vault or a similar secret store should be used to pass Authorization keys between deployments if required.
...
- An AVS Scale operation is not instantaneous, and you need to consider the time it takes to add an additional node to an existing cluster.
