...
- Define ClusterAutoScaler and MachineAutoScaler to scale machines when your cluster runs out of resources to support more deployments. Please note that the [ClusterAutoscaler](https://docs.redhat.com/en/documentation/openshift_container_platform/latest/html/machine_management/applying-autoscaling#cluster-autoscaler-about_applying-autoscaling) considers resources of ALL nodes, including control plane nodes, on the cluster. As identified in the [responsibility matrix](https://learn.microsoft.com/en-us/azure/openshift/responsibility-matrix#change-management), in order to ensure SLA, SRE may scale up the cluster's control plane nodes. This will reduce the remaining capacity for auto scaling worker nodes. Please consider this when defining the ClusterAutoscaler.
