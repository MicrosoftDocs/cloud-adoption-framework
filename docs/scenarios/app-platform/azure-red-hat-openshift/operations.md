...
- Define `ClusterAutoscaler` and `MachineAutoscaler` resource definitions to describe how adjust your cluster when your it runs low on resources so that you can support continued operations. The [`ClusterAutoscaler`](https://docs.redhat.com/en/documentation/openshift_container_platform/latest/html/machine_management/applying-autoscaling#cluster-autoscaler-about_applying-autoscaling) considers resources of all nodes, including control plane nodes, on the cluster. As you scale up the cluster's control plane nodes you reduce the remaining capacity for auto scaling worker nodes.
