...
- Active Directory trust configuration and DNS configuration are key to a successful IdM implementation. Note that when configuring Identity Management and Active Directory, the both Active Directory Servers and IdM servers need to reside in their own DNS domains or subdomains. This is due to the nature of Kerberos service records and Kerberos service discovery. This is not a limitation of Active Directory or Identity Management. Private IP DNS resolution for Azure-based instances is recommended in the [Azure Cloud Adoption Framework](https://learn.microsoft.com/en-us/azure/cloud-adoption-framework/ready/azure-best-practices/dns-for-on-premises-and-azure-resources).
...
- Back up your IdM resources. Although Identity Management is typically deployed in a self-managed multi-master replica configuration, ensuring proper backups of systems and data is critical. Use Identity Management hidden replicas to implement full offline backups without interrupting service availability. Use Azure Backup for encrypted backup facility.   
