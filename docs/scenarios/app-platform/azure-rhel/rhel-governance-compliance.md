...
Efficient and effective governance and compliance in a cloud environment is an enormous task. This article describes considerations and recommendations for Red Hat Enterprise Linux (RHEL) operating system images and instances.

Compliance for RHEL deployments in Azure refers to the methods that you can use to define, measure, and report on how systems conform to a rule, such as a specification, policy, or standard. Your organization likely has requirements for how you use a system.Governance refers to the structures and processes that you use to define the specifications that you need to meet. Governance also includes how you enforce those specifications and how you remediate misalignment.

##

Organizations, especially those in regulated industries, often need an authority to operate (ATO) to install and use software in their environments. Part of this process is to evaluate the software against a security requirements guide (SRG), which is a set of technical controls such as the controls in the National Institute of Standards and Technology (NIST) [security and privacy controls for information systems and organizations](https://csrc.nist.gov/pubs/sp/800/53/r5/upd1/final).
...
The Security Content and Automation Protocol (SCAP) provides these checklists. SCAP is a suite of specifications for exchanging security automation content, such as definitions of checks and automation methods. This content is used to assess configuration compliance and to detect the presence of vulnerable versions of software. Red Hat works with NIST and the MITRE corporation to write and publish content that scanning tools use to evaluate and report on a wide variety of compliance standards for the RHEL operating system and other Red Hat software.

Red Hat also contributes to the open-source projects that develop the standard languages and tools to implement the checklists. The Open Security Content and Automation Protocol (OpenSCAP) open project is the integration point for these efforts with Red Hat software. The OpenSCAP project combines standardized components to implement the tools that you can use to create, maintain, scan, report, and analyze the results of compliance definitions. The compliance definitions are written in Open Vulnerability and Assessment Language (OVAL) and eXensible Configuration Checklist Description Format (XCCDF). Both formats are represented in XML. Think of OVAL as a means to define and measure a logical assertion about the state of an endpoint system. Think of XCCDF as a means to express, organize, and manage those assertions into security policies. The scanner that OpenSCAP developed can consume both of these document types.  

Red Hat founded and contributes to the [compliance as code open-source project](https://www.redhat.com/blog/compliance-code-extending-compliance-automation-process-improvement), which delivers content in SCAP, Ansible, and other formats. You typically use SCAP for measuring and reporting and use Ansible for remediation.
...
- CIS RHEL 9 Benchmark for Level 2 - Server
...
- [DRAFT] DISA STIG with GUI for RHEL 9

The Red Hat Security Response Team provides a published stream of known Common Vulnerabilities and Exposures (CVE) information for Red Hat products in OVAL format. Red Hat recommends that you use these resources as part of your compliance implementation in Azure.
