...
***Workload landing zone*** - The workload landing zones are for workloads. The grey boxes outline the workload landing zones. There are two workload landing zone subscriptions in the diagram below (Landing zone A1 and Landing Zone A2). The image only shows one workload landing zone in detail (Landing Zone A2).
