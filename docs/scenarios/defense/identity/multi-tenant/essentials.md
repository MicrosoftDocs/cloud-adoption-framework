...
Azure AD has no domain controllers, organizational units, group policy objects, domain/forest trusts, or FSMO roles. Azure AD is Software-as-a-Service, is accessed via RESTful APIs and modern authentication and authorization protocols and uses a flat structure with resource-based delegation. Each instance of Azure AD is a [highly available](/azure/active-directory/fundamentals/active-directory-architecture) data store containing identity, policy, and configuration objects replicated across Azure regions. An Azure AD instance provides data redundancy for critical defense information.
...
**Protocol support.** Azure AD applications don't use Kerberos, NTLM, or LDAP. They must use modern open protocols intended for use over the internet, such as [OpenID Connect](h/azure/active-directory/develop/v2-protocols-oidc), [OAuth 2.0](/azure/active-directory/develop/v2-oauth2-auth-code-flow), [SAML 2.0](/azure/active-directory/develop/saml-protocol-reference), and [SCIM](/azure/active-directory/fundamentals/sync-scim).
...
Azure AD isn't just for Microsoft applications and services. Azure AD can be the identity provider for any application, cloud service provider, SaaS provider, or identity system that implements the same protocols. This support flexibility provides interoperability with allied defense forces and contractors.

Azure AD is a common [policy enforcement point (PEP)](https://csrc.nist.gov/glossary/term/policy_enforcement_point) and [policy decision point (PDP)](https://csrc.nist.gov/glossary/term/policy_decision_point) in zero trust architectures. [Entra Identity Governance](https://learn.microsoft.com/en-us/azure/active-directory/governance/identity-governance-overview) manages user access and automate access lifecycle. Azure AD [Conditional Access](/azure/active-directory/conditional-access/overview) enables fine-grained authorization based on environment details and context for each authentication. It can use user attributes, credential strength, application attributes, user and sign-in risk, device health, location, and more. For more information, see [zero trust security](/azure/security/fundamentals/zero-trust).
...
Many Azure resources can use a [managed identity](/azure/active-directory/managed-identities-azure-resources/overview) to interact with other resources. The managed identity is a security principal in the Azure AD tenant. You can grant permissions to the managed identity and authorize it to access APIs protected by Azure AD, such as Microsoft Graph. When an Azure resource uses a managed identity, the resource's identity is a [service principal object](https://learn.microsoft.com/en-us/azure/active-directory/develop/app-objects-and-service-principals?tabs=browser#service-principal-object) in the same Azure AD where its subscription is attached.
