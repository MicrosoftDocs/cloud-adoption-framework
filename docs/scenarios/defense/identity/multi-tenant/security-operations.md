...
This scenario combines [Azure Lighthouse](/azure/lighthouse/overview), [Entra External ID](/entra/external-id/external-identities-overview), [Entra Privileged Identity Management](/entra/id-governance/privileged-identity-management/pim-configure), and [multi-tenant management in Microsoft Defender XDR](/microsoft-365/security/defender/mto-overview).

**Use Azure Lighthouse to manage Microsoft Sentinel in secondary tenants.** You need to use [Azure Lighthouse](/azure/lighthouse/overview) to manage Microsoft Sentinel workspaces in subscriptions attached to secondary tenants. Azure Lighthouse enables multi-tenant management with scalability, higher automation, and enhanced governance across resources.
...
> [!IMPORTANT]
> There are two ways to use Entra Privileged Identity Management with Azure Lighthouse. You can use PIM for groups to elevate membership to an Entra security group with a permanent authorization configured in Azure Lighthouse as described in the previous section. Another option is to configure Azure Lighthouse with [eligible authorizations](/azure/lighthouse/how-to/create-eligible-authorizations). See [onboard a customer to Azure Lighthouse](/azure/lighthouse/how-to/onboard-customer) for more information.

## Configure centralized security operations

To set up centralized security operations for a multi-tenant environment, you must configure Azure Lighthouse, Entra External ID, and Entra Privileged Identity Management. A security operator in the primary tenant can use a single identity secure multiple tenants. They sign in once, elevate their access with PIM, monitor resources across tenants and services, and respond to threats across tenants (*see figure 1*).
...
Security operators use the multi-tenant security operations setup (*see figure 1*) to secure multiple tenants. They can monitor, investigate, and respond to threats across Microsoft 365 and Azure across Microsoft Entra tenants (*see figure 2*).

:::image type="content" source="./images/lighthouse-operations.png" alt-text="Diagram showing multi-tenant security operations for Sentinel, Microsoft Defender XDR, Defender for Cloud." lightbox="./images/lighthouse-operations.png" border="false":::
