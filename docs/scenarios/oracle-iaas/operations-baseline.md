...
An architecture that meets business continuity and disaster recovery (BCDR) objectives provides the solid foundation. Management and monitoring capabilities strengthen your architecture to discover failures and abnormalities and ensure health of your mission critical workloads. BCDR approaches for Oracle on Azure IaaS workloads are covered in detail in [this article](./oracle-disaster-recovery-oracle-landing-zone.md). This article focuses on monitoring and diagnostics of your Oracle workload on Azure IaaS.

Azure provides native monitoring capabilities, which is recommended to leverage for Oracle Workloads on Azure IaaS. By help of Azure monitor you will be equipped to observe metrics, such as consumed IOPS or bandwidth on your virtual machine(s), managed disks and/or Azure NetApp Files.
Additionally, we recommend leveraging Oracle specific monitoring tools to ensure your databases are operating at the required levels. These include Oracle Enterprise Manager and also your AWR/Statspack reports.
...
Data and OS disk performance metrics are important because the Oracle database files are stored on the managed disks. Consider the cumulative IOPS and throughput of data and OS disks when disk striping technologies are used such as Oracle Automatic Storage Management (classic deployment model) or Linux Logical Volume Manager (LVM).

The Oracle administrator should monitor disk performance metrics related to Input/Output like the following metrics:
...
While selecting the appropriate VM SKU, use the table and column **Max uncached disk throughput: IOPS/MBps** to see how multiple managed disks attached to the Virtual Machine can cumulatively provide a higher combined IOPS and throughput. Note if the database I/O requirements during peak load are higher than the Virtual Machine max uncached disk throughput, the Virtual Machine I/O operations can be throttled. Alternatively, if there's insufficient IOPS and/or storage throughput per disk, throttling can occur at the disk level.
...
|:---|:---|
|OS Disk IOPS Consumed Percentage | OS Disk IOPS Consumed Percentage > 75 |
|Data Disk IOPS Consumed Percentage | Data Disk IOPS Consumed Percentage > 75 |
...
### Oracle workload monitoring by Oracle Enterprise Manager
