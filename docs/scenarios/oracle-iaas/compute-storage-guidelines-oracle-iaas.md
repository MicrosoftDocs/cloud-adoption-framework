...
Azure offers certain VM sizes with [constrained VM](https://learn.microsoft.com/azure/virtual-machines/constrained-vcpu) vCPU count  to reduce the cost of software licensing. Azure reduces cost while maintaining the same memory, storage, and I/O bandwidth required by the Oracle database workloads.  For example, a “Standard_M16-4 ms” VM has the same performance specifications as a “Standard_M16ms” VM but since it has only 4 available CPUs, it  provides significant Oracle license savings for the customer.  
...
When selecting a VM size on Azure, remember that there are limits on the following resources:  
...
Network throughput is important especially when Oracle Data Guard is used since online redo logs continually replicate to the standby database instance.

## Key guidelines for storage

Azure offers various [storage options](https://learn.microsoft.com/azure/virtual-machines/workloads/oracle/oracle-storage), each suited for different use cases for your Oracle workload on Azure. The following section provides some advice to select the right storage:
...
- While Ultra Disks provide superior performance, they also have some constraints:  
    ...
    - No Direct Support for Availability Sets or Azure Disk Encryption: Ultra Disks don't support Availability Sets or Azure Disk Encryption, so consider other options for high availability and encryption. The only infrastructure redundancy options currently available to ultra disks are availability zones. VMs using any other redundancy options can't attach an ultra disk.
...
Azure ultra disks offer up to 32 TiB per region per subscription by default, but ultra disks can support higher capacity by request.
...
While Premium SSDs offer reliable and consistent performance, there are a few constraints:
   ...
   - Size and Performance Dependency: The size of the disk determines the performance of a Premium SSD in terms of IOPS and throughput. If you need more IOPS or throughput, you need to provide a larger disk, which could lead to unused capacity.
...
- ANF-files 
    ...
    - Efficient storage utilization: With features like data de-duplication, compression, and thin provisioning, NetApp offers efficient storage utilization. Efficient storage utilization can lead to significant cost savings for businesses running Oracle databases.
    - Oracle certification: NetApp solutions are Oracle-certified, meaning they have been tested and verified to work efficiently with Oracle products. This certification gives businesses confidence in the compatibility and performance of the combined solution.  
...
The following articles provide guidance for specific points throughout the cloud adoption journey to help you succeed in adopting Oracle to Azure VMs on landing zone accelerator.
