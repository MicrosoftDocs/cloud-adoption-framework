...
| SAP HANA | Raw data or logical objects| SAP HANA | HANA database row and column tables, information models (analytic and calculation views) and custom query all support full load. You can also extract changes with manual watermarking using timestamp or unique ID column | ODBC calls to HANA database|  Basic, 
Windows (SSO)" |	Prerequisites | Yes,  Fast with built-in parallel loading based on configurable partitioning |	Large |
