...
| **SAP Source** | **Type of data** | **Azure's**** Connector **|** Supported objects and mode **|** Protocol and connection **|** Authentication **|** Additional information **|** ParallelisationParallelization and performance **|** Data volumes** |
| --- | --- | --- | --- | --- | --- | --- | --- | --- |
| SAP applications based on NetWeaver AS ABAP such as SAP ECC, S/4HANA, CRM, SRM, APO?, BW on HANA, BW/4HANA
 | Raw data | [SAP CDC](https://learn.microsoft.com/en-us/azure/data-factory/connector-sap-change-data-capture)(\*_via SLT_) | All transparent, pooled and cluster tables support full load and CDC.
Certain views (SE11) support full load and CDC.
 | RFC calls to NetWeaver application and message server | BasicSNC
 | [Prerequisites](https://learn.microsoft.com/en-us/azure/data-factory/sap-change-data-capture-prerequisites-configuration) | Yes
Fast with built-in parallel loading based on configurable partitioning.
 | Large |
| SAP applications based on NetWeaver AS ABAP such as SAP ECC, S/4HANA, CRM, SRM, BW on HANA, BW/4HANA | Logical objects | [SAP CDC](https://learn.microsoft.com/en-us/azure/data-factory/connector-sap-change-data-capture) | **ABAP CDS views** support full load, many of them support delta (e.g., based on time stamps, or triggers)
...
| SAP applications based on NetWeaver AS ABAP such as SAP ECC, S/4 HANA, BW on AnyDB, BW on HANA
_\*BW/4HANA not supported_
 | Raw data | [SAP Table](https://learn.microsoft.com/en-us/azure/data-factory/connector-sap-table?tabs=data-factory) | All transparent, pooled and cluster tables and views (SE11) support full load. You can also extract changes with manual watermarking using timestamp or unique ID column. | RFC calls to NetWeaver application and message server | BasicSNC
 | [Prerequisites](https://learn.microsoft.com/en-us/azure/data-factory/connector-sap-table?tabs=data-factory#prerequisites) | Yes
Fast with built-in parallel loading based on configurable partitioning | Large |
| BW on AnyDB, BW on HANA
_\*BW/4HANA not supported_
 | Logical objects | [SAP BW Open Hub](https://learn.microsoft.com/en-us/azure/data-factory/connector-sap-business-warehouse-open-hub) | BW objects exposed via Open Hub Destination (OHD) local table such as DataSources, DSO, InfoCube, MultiProvider, CompositeProvider, Query (if exposed as InfoProvider) support full load. Delta is supported if the OHD supports delta.
 | RFC calls to NetWeaver application and message server | Basic
 | [Prerequisites](https://learn.microsoft.com/en-us/azure/data-factory/connector-sap-business-warehouse-open-hub#prerequisites)
Create SAP OHD in SAP BW to expose data. | Yes
Fast with built-in parallel loading based on OHD specific schema | Large |
| BW on AnyDB, BW on HANA
_\*BW/4HANA not supported_
 | Logical objects | [SAP BW via MDX](https://learn.microsoft.com/en-us/azure/data-factory/connector-sap-business-warehouse?tabs=data-factory) | BW InfoCubes and QueryCubes (including BEx queries) support full load. Delta is not supported.

 | RFC calls to NetWeaver application server | Basic | [Prerequisites](https://learn.microsoft.com/en-us/azure/data-factory/connector-sap-business-warehouse?tabs=data-factory#prerequisites) |

Slower | Small |
| SAP ECC, S/4 HANA | Raw data or logical objects | [SAP ECC](https://learn.microsoft.com/en-us/azure/data-factory/connector-sap-ecc?tabs=data-factory)_(\*via SAP Gateway)_ | Any objects exposed by OData services such as SAP tables, views, BAPI objects, extractors, data or IDOCs sent to SAP PI that can be received as OData via relative adapters, support full load. You can also extract changes with manual watermarking using timestamp or unique ID column.
 | HTTP (OData) calls to NetWeaver application and message server
 | Basic | [Prerequisites](https://learn.microsoft.com/en-us/azure/data-factory/connector-sap-ecc?tabs=data-factory#prerequisites)
Set up SAP Gateway, activate OData service, and expose entities.
 |

Slower | Small |
| SAP HANA | Raw data or logical objects | [SAP HANA](https://learn.microsoft.com/en-us/azure/data-factory/connector-sap-hana?tabs=data-factory) | HANA database row and column tables, information models (analytic and calculation views) and custom query all support full load. You can also extract changes with manual watermarking using timestamp or unique ID column. | ODBC calls to HANA database | BasicWindows (SSO) | [Prerequisites](https://learn.microsoft.com/en-us/azure/data-factory/connector-sap-hana?tabs=data-factory#prerequisites) | Yes
Fast with built-in parallel loading based on configurable partitioning | Large |
| SAP Cloud for Customer (SaaS CRM) | Raw data or logical objects | [SAP C4C](https://learn.microsoft.com/en-us/azure/data-factory/connector-sap-cloud-for-customer?tabs=data-factory) | This is a bi-directional connector; it can copy data from and to SAP C4C.
Objects that can be exposed by OData services from SAP C4C are supported. | HTTP (OData) calls to SaaS service
 | Basic |
 |
