...
Before trying to integrate SAP systems with Power Platform, you must have the Power Platform and SAP systems deployed and configured. Power Platform doesn't have any specific requirements (except connectors and on-premises data gateway) to connect with SAP. The connectors are part of Power Platform, and thus, the section only focuses on the design and deployment of on-premises data gateway, which is the middleware component needed for Power Platform and SAP integration. The usage of an API gateway solution for non-RFC calls is optional but recommended, Azure API Management is used in this solution for that component.
