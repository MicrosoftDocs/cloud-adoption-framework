...
Choose load-balancing technology based on traffic type and required features. Azure Standard Load Balancer handles Layer 4 (TCP/UDP) traffic for high-performance east–west and north–south scenarios. Azure Application Gateway handles Layer 7 (HTTP/HTTPS), provides TLS termination and end-to-end TLS, URL-based routing, and a Web Application Firewall (WAF). Azure Front Door provides global HTTP/HTTPS load balancing and edge acceleration. SAP Web Dispatcher provides application-layer routing and session affinity for ABAP/Java workloads. Use third-party network virtual appliances when you require additional protocol or inspection features.

Design patterns combine solutions for high availability and scale. Load balancers direct client traffic to virtual machines (VMs) in the application subnet. For example, use SAP Web Dispatcher with an internal Standard Load Balancer to distribute backend VM traffic and provide scale-out capacity. Use Application Gateway or Front Door for internet-facing HTTP/HTTPS traffic where you require TLS termination, WAF protection, or global failover. When you terminate TLS at the edge, configure end-to-end TLS or re-encrypt traffic between the edge and SAP application servers to meet compliance or sensitivity requirements.
