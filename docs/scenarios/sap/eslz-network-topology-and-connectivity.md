...
Customers approaching a RISE implementation will normally have two options when it comes to connectivity: Site-to-site VPN or Vnet peering. For those customers without any prior Azure workloads, S2S might be the easier option. However, customers who envision adopting Azure as a strategic platform might be interested in setting up a proper Azure landing zone and employ Vnet peering to the SAP RISE tenant. For these scenarios, a simplified landing zone like the [CAF Migration Landing Zone](https://learn.microsoft.com/en-us/azure/governance/blueprints/samples/caf-migrate-landing-zone/) could be a good option. This blueprint can easily be adopted to the specific customer requirement, with specific focus on the Vnet parameters planned by the customer.

Deploying cross-tenant Vnet peering to the SAP RISE tenant also implies some further work on the customer side: careful planning of the Vnet architecture is needed to ensure there are no overlapping CIDR ranges; the customer must set up a virtual WAN; and DNS peering must be properly done as per the document referred to above (Integrating Azure with SAP RISE managed workloads). Also, pay specific attention to scenarios where the customer also intends to mix S2S VPN connections into the VWAN solution, as this can [add complexity](https://learn.microsoft.com/en-us/azure/expressroute/how-to-configure-coexisting-gateway-portal?toc=%2Fazure%2Fvpn-gateway%2Ftoc.json#limits-and-limitations).
