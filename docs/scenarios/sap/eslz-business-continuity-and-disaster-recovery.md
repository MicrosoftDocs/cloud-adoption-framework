...
> [!NOTE]
> This document describes single-sid deployment, but Azure also supports multiSID-cluster deployments. A maximum of five SAP sids can be deployed in a Linux cluster running in Azure.
...
- Use the [E-series](/azure/virtual-machines/e-series) or [M-series](/azure/virtual-machines/m-series) SKU for anydb. E-series and M-series SKUs are optimized for memory and provide a consistent CPU-to-memory ratio.
