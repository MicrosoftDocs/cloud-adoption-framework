...
- In cost-conscious deployments consider [disk bursting](https://docs.microsoft.com/en-us/azure/virtual-machines/linux/disk-bursting#disk-level-bursting) feature for non-production workloads.
- Every Azure premium disk come with IOPS and throughout limit, consider striping disk instead of using single large disk for a cost-conscious deployment. Further details about stripe sizes can be found [here](https://docs.microsoft.com/en-us/azure/virtual-machines/workloads/sap/hana-vm-operations-storage).
...
- [Read-Only](https://docs.microsoft.com/en-us/azure/virtual-machines/premium-storage-performance#disk-caching) cache must be enabled for the data disks holding database data files for AnyDB deployment. Read-Only cache can only be enabled on disks smaller than 4095GB.
