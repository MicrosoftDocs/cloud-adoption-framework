...
Companies are scattering resources across on-premises, multiple clouds, and the edge. And, customers have three common needs that we hear about, often:

1. You need to have visibility into the health of all your existing and future infrastructure and applications through a single pane of glass.
2. From an IT point of view, while your organization understands the need for implementing a governance standard, it finds it hard to integrate on-premises policies and updates with cloud infrastructure.
3. Since there are often different app development teams in organizations, there is a wide range of skills across on-premises and cloud. Customers are looking for consistent interoperability between the two so they can unify development practices.
...
There is another concern associated with hybrid and multicloud adoption after the workloads are released and in production. When organizations attempt to provide operations management support to workloads in new environments, they must often swiftly rethink their practices. Existing platforms of operations management were not built for these environments, nor most existing operations management policies and processes. To account for deviations in cloud environments, companies often end up with disparate operations tooling and operations practices, multiplying the cost of operations by the number of cloud environments supported.
