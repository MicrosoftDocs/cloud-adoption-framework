...
- Be aware of the differences between [Horizontal Pod Autoscaler (HPA) and Cluster autoscaler](/azure/aks/concepts-scale)
- Consider securing traffic between pods using [network policies and the Azure policies plugin](/azure/aks/use-network-policies)
- To help troubleshoot your application and services running on AKS, you may need to view the logs generated by control plane components. You may want to [enable logs](/azure/aks/view-control-plane-logs) since it is not enabled by default.
