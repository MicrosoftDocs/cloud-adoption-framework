...
- Figure out if you need a [financially backed SLA](/azure/aks/uptime-sla) for your Kubernetes API server endpoint.
...
- Whenever possible, [remove service state from inside containers](/azure/aks/operator-best-practices-multi-region#remove-service-state-from-inside-containers). Instead, use an Azure platform as a service (PaaS) that supports multi-region replication.
...
- Regularly run the latest version of [kube-advisor open source tool](/azure/aks/operator-best-practices-scheduler#regularly-check-for-cluster-issues-with-kube-advisor) to detect issues in your cluster.
...
- Using [split TCP-based anycast protocol](/azure/aks/operator-best-practices-multi-region#application-routing-with-azure-front-door-service), Azure Front Door Service ensures that your end users promptly connect to the nearest Front Door Point of Presence. Other features of Azure Front Door Service include:
