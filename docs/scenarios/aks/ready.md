...
**Enterprise-scale landing zones:** The enterprise-scale for AKS includes a reference implementation to deploy an enterprise-scale landing zone to support your AKS platform. The enterprise-scale landing zone deploys the same AKS baseline, but it does so in alignment with the enterprise-scale landing zone implementation.
