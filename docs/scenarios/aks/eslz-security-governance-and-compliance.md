...
- Scan your images for vulnerabilities, either with [Azure Defender for container registries](/azure/security-center/defender-for-container-registries-introduction) (note that at the time of this writing Azure Defender image scanning is not compatible with ACR endpoints, see [Connect privately to an ACR using private link](/azure/container-registry/container-registry-private-link)) or any other image scanning solution.
- Monitor and enforce configuration by using the [Azure Policy add-on for Kubernetes](/azure/aks/use-azure-policy).
