...
At creation, the Microsoft Entra ID system-assigned identity can only be used to update the status of the Azure Arc-enabled servers (for example, the 'last seen' heartbeat). In granting this identity access to Azure resources, applications on your server are able to use it to access Azure resources (for example, to request secrets from a Key Vault). You should:
