...
- Additionally, while using the internet, you can choose to use the [Azure Arc Gateway](https://learn.microsoft.com/azure/azure-arc/servers/arc-gateway) to reduce number of endpoints that the proxy must allow.
...
To reduce the total number of URLs required in the firewall or proxy, determine if the [Azure Arc Gateway](https://learn.microsoft.com/azure/azure-arc/servers/arc-gateway) would be of benefit.
