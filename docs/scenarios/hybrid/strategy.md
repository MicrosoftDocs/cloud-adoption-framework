...
description: Azure hybrid and multicloud strategy. Learnd to use Azure as unified control plane for on-premises, edge, AWS, and Google cloud to improve global resilience, compliance, and cost efficiency.
...
|----------------------------|------------------------------------------------------------------------------------------------------------------------------------------------------------|
| Hybrid & multicloud management | **Azure Arc** – Project eligible servers, supported Kubernetes clusters, Azure Local infrastructure, and selected data services into Azure for a unified control plane. This enables central management and policy enforcement across on-prem and other clouds where Arc support exists. |
| Identity & access management  | **Microsoft Entra ID** – Use it as the unified identity platform across all environments via synchronization or federation. This provides single sign-on and unified credential management for Azure, on-prem AD, AWS, Google Cloud, and SaaS apps. Adopt conditional access and role-based access control (RBAC) patterns consistently across environments. |
| Observability & monitoring    | **Azure Monitor** – Consolidate logs, metrics, and traces from every environment into Azure Monitor. Use Log Analytics workspaces and Azure Monitor agents/Arc to ingest data from on-prem and other clouds. Complement with Azure Automation/Automanage for routine ops (backup, patching) tasks across resources where Automanage for Arc-enabled servers is supported. |
| Container orchestration       | **Azure Kubernetes Service (AKS)** – Standardize on AKS for containerized workloads. Use AKS in Azure and AKS enabled by Azure Arc (or Arc-enabled Kubernetes on other infra) so that Kubernetes clusters everywhere are managed consistently. Also consider Azure Container Apps for serverless containers where appropriate. |
...
| On-premises infrastructure    | **[Azure Local](/azure/azure-local/overview)** – For any new on-prem hardware investment or private cloud needs, use Azure Local (Azure Stack HCI) to ensure it seamlessly integrates with Azure services. Azure Local allows running VMs and even Azure services on-prem, enabled by Azure Arc for unified management. |
| Security & compliance         | **Microsoft Defender for Cloud** – Use this for unified Cloud Security Posture Management and threat protection across Azure, AWS, and GCP resources. It identifies misconfigurations and threats in any cloud. Combine Azure Policy (for Azure and Arc-enabled resources) with Defender for Cloud multicloud connectors (AWS/GCP) to enforce and surface governance and posture across environments. Also consider Microsoft Sentinel for SIEM across logs from all environments. It's a cloud-native SIEM and SOAR that correlates logs from Azure, Arc-enabled resources, and multicloud connectors.|
