...
- When [Azure Arc-enabled Kubernetes agents](/azure/azure-arc/kubernetes/conceptual-agent-overview) traffic is going through the firewalls or proxy servers, create a source and destination object groups and/or tags to simplify outbound internet traffic rules to support additional URL whitelist for Azure Arc extensions.
