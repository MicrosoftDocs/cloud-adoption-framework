...
- Consider using dedicated Azure Log Analytics workspace per region to collect logs and metrics from Azure Arc-enabled Kubernetes cluster for monitoring and reporting across multiple clusters in a specific environment. Review [Designing your Azure Monitor Logs deployment](/azure/azure-monitor/logs/design-logs-deployment) for additional details.
- Consider Azure Monitor [IT service management (ITSM) connector integration](/azure/azure-monitor/alerts/itsmc-overview) with your organization tools to raise incidents for Azure Monitor alerts and track issue resolution.
