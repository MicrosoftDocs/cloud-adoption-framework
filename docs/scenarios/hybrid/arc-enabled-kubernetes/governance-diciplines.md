...
Before going through this document it is important to understand governance principles to better align with organization security policies, also make sure to review [Enterprise-scale for AKS](/azure/cloud-adoption-framework/scenarios/aks/enterprise-scale-landing-zone) to understand the relevant concepts of Enterprise Scale in a container environment.
**JP Comment: Any specific sections we can call out within enterprise-scale for AKS, like Governance? The broad enterprise scale covers a lot of AKS specifics that may not be applicable to Arc.**
...
- **Agent Provisioning:** Define a strategy for provisioning the Azure Arc-enabled Kubernetes agent and use the least privilege principle when creating the onboarding service principal. Consider a method of automation for bulk enrollment.
...
**JP Comment: The part of the sentence "create GitOps configurations and enforce governance policies." should possible be removed from this section as I don't think it is applicable here**
...
- **Enable and secure GitOps configurations:** GitOps configurations enforce the desired state of the system and is an important tool for compliance tracking of Arc-enabled Kubernetes clusters. When working with GitOps configurations consider securing access to your source control system with appropriate network and access controls.
**JP Comment: Add link to CI/CD workflow CDA for reference(./cicd-workflow.md)**
...
**JP Comment: LInk to serivces observability cda (./services-observability.md)**
...
**JP Comment: Consider changing "which basically has the power to do anything on Arc-enabled Kubernetes" to something like "which can have a cluster-wide impact.**
...
Integrating with Azure AD also enables the use of Azure AD advance security capabilities:
