...
Service Observability is important, it allows you to understand performance and issues that are occurring with distributed and cloud systems that are based on dynamic architectures. Implementing a solution to achieve service observability will gain the following benefits:
...
- The service meshes observability impact by increasing the breadth of coverage to all services which are part of the mesh.
- Features that can help increase adoption of observability by auto-instrumenting all services.
- Choose a service mesh that comes with an ability to scrape metrics and collect logs that can surface into your monitoring solution. Your monitoring solution should be able to support ingestion and process of metrics and logs based on your needs.
...
- Get an overview of all services in your mesh, providing you critical, service-level metrics on three of the four golden signals of monitoring: latency, requests, and errors.
