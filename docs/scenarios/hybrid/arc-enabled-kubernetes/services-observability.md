...
- **Metrics** are numerical values that are collected at regular intervals and describe some aspect of a system at a particular point in time. See [Server requests screenshot](./azure/azure-monitor/essentials/media/data-platform-metrics/metrics-explorer.png) from Azure Monitor as a visualization example of a metric charted over a time period.
...
- **Distributed tracing** is a diagnostic technique that helps users localize failures and performance issues within applications, especially those that may be distributed across multiple machines or processes. This technique tracks requests through an application, correlating together work done by different application components and separating it from other work the application may be doing for concurrent requests. See an [End-to-end transaction screenshot](./azure/architecture/microservices/images/monitoring/transaction.png) as an example visualization of distributed tracing using Application Insights.
