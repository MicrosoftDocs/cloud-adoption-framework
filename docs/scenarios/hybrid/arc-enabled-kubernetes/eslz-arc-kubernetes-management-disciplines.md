...
- Azure [Arc-enabled Kubernetes extensions](/azure/azure-arc/kubernetes/extensions) require additional URLs to whitelist on firewalls or proxy servers. Depending on the extensions being installed, consider planning to accommodate additional URLs to whitelist to minimize change management timelines.
...
- Consider using dedicated Log Analytics workspace per region to collect logs and metrics from Azure Arc-enabled Kubernetes cluster to monitor and report across multiple cluster in a specific environment. Review [Designing your Azure Monitor Logs deployment](/azure/azure-monitor/logs/design-logs-deployment) for additional details.
