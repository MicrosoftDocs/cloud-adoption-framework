...
- Consider using the [Azure Arc-enabled Kubernetes cluster connect](/azure/azure-arc/kubernetes/conceptual-cluster-connect) feature to connect to the apiserver without requiring any inbound port to be enabled on the firewall. Review the [network connectivity critical design area](./eslz-arc-kubernetes-network-connectivity.md) to understand how this feature works.
