...
- Azure [Arc-enabled Kubernetes extensions](/azure/azure-arc/kubernetes/extensions) require additional URLs to allow on firewalls or proxy servers. Depending on the extensions being installed, consider planning to accommodate additional URLs to allow on firewalls or proxy servers to minimize change management timelines.
...
- Consider using a dedicated Log Analytics workspace per region to collect logs and metrics from Azure Arc-enabled Kubernetes cluster to monitor and report across multiple clusters in a specific environment. Review [Designing your Azure Monitor Logs deployment](/azure/azure-monitor/logs/design-logs-deployment) for additional details.
