...
1. **Plan for agent-based architectures.** Agent systems represent the evolution toward autonomous AI that collaborates to complete complex business processes. These systems use multiple specialized agents that work together, moving beyond single-model solutions to create more capable implementations. You must assess how your current technology approach adapts to support agent coordination and multi-system workflows. Review [What are agents?](/azure/ai-foundry/agents/overview#what-is-an-ai-agent) to understand agent capabilities.

2. **Adopt open protocols for AI interoperability.** Open protocols ensure AI systems communicate effectively across different platforms and reduce dependency on single vendors. These protocols enable data sharing and system integration while maintaining flexibility for future technology changes. You must evaluate protocols like [NLWeb](https://github.com/microsoft/NLWeb?tab=readme-ov-file#what-is-nlweb) for web object serving and Model Context Protocol for cross-system data ingestion to ensure your AI architecture supports interoperability.
