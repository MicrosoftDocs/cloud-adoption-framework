...
- *Control the models you use.* Use Azure Policy to manage which specific models your teams are allowed to deploy from the Azure AI Foundry model catalog. You have the option to use a [built-in policy](/azure/ai-studio/how-to/built-in-policy-model-deployment) or create a custom policy tailored to your needs. Since this approach uses an allowlist, begin with an *audit* effect. The *audit* effect allows you to monitor the models your teams are using to use without restricting deployments. Switching to a deny effect too early can unnecessarily impede workload development.
