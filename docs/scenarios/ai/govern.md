...
This article offers recommendations for developing your AI governance framework. It assumes you are already familiar with the governance processes defined in [CAF Govern](/azure/cloud-adoption-framework/govern/) and does not repeat the common cloud governance practices found there. Additionally, it presumes you have established governance policies at the management group level for your AI workload types, such as internet-facing (“online”) and internal (“corporate”) workloads, which was covered in [AI Ready](ready.md#prepare-ai-resource-organization).
...
- *Establish a policy to onboard new models.* Use sandbox environments to experiment with different models, encouraging innovation and testing. When moving a model to production, implement a formal onboarding process that includes justification, validation, and approval. Review existing models in the production catalog to prevent duplication and maintain streamlined operations.
...
- *Create a policy to shutdown compute instances.* Define and enforce a policy for AI resources to use the auto shutdown feature for virtual machines and compute instances in Azure AI Studio and Azure Machine Learning for certain activities, such as prompt flow. The auto shutdown is primarily applicable to non-production environments.
...
- *Automate compliance.* Use [Microsoft Purview Compliance Manager](https://docs.microsoft.com/microsoft-365/compliance/compliance-manager-overview) to assess and manage compliance across cloud environments. Use the applicable [regulatory compliance initiatives](/azure/governance/policy/samples/#regulatory-compliance) in Azure Policy for your industry. Apply additional policies based on the AI services you’re using, such as [Azure AI Studio](/azure/ai-services/security-controls-policy), [Azure Machine Learning](/azure/machine-learning/policy-reference), and [Azure Virtual Machines](/azure/virtual-machines/policy-reference).
...
- *Implement version control for RAG data.* Establish a version control process for Retrieval-Augmented Generation (RAG) data. This ensures that any changes to the underlying data or its structure are tracked and can be reverted if necessary, maintaining consistency across deployments.
...
|---|---|---|---|
| R001    | Non-compliance with data protection laws  | Use Microsoft Purview Compliance Manager to evaluate data compliance.                                  | All AI development and deployment must comply with data protection laws by implementing the Security Development Lifecycle.          |
...
| R009    | Unexpected costs              | Use Azure Cost Management to track CPU, GPU, memory, and storage usage to ensure efficient resource utilization and prevent cost spikes. | Unexpected costs must be managed by monitoring and optimizing resource usage and implementing automated detection of cost overruns.      |
