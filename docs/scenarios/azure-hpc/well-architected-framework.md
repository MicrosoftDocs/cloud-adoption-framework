...
In order to support critical HPC workloads, we have the latest recommendation for the ExpressRoute High Availability (ER HA). This configuration ensures that the customer has connectivity to Azure in the event of an ER outage in a single region. Even in a single site high availability setup where the customer has a redundant ExpressRoute connection, this does not protect the customer against single edge site downtime. We recommend, for mission critical workloads, that the customer enable two ExpressRoute circuits in two different ER edge site locations to provide maximum redundancy. This setup requires provisioning two ER circuits in the Azure Portal for the two different ER edge site locations, then connecting both ExpressRoute circuits to the same hub virtual network in Azure. The two different edge site locations would be in the same region, as several [metros](https://learn.microsoft.com/azure/expressroute/expressroute-locations-providers#global-commercial-azure) have multiple ExpressRoute sites. You can work with your provider or a different provider in order to configure the second ExpressRoute site. It is critical to ensure that the second connection is working by failing traffic over to the second location and performing regular drills to ensure connectivity. Please see the diagram to illustrate the maximum resiliency architecture:

![Diagram showing ExpressRoute architecture for mission critical workloads.](./media/hpc-expressroute-ha-sku.png)
The diagram pictured illustrates a multi-homed ExpressRoute gateway. The left side depicts the primary locations connection and the right side depicts the secondary location connection. Enabling two connections at two facilities allows for a redundancy in connection that can enable business to continue in the event of a disaster at the primary location.
