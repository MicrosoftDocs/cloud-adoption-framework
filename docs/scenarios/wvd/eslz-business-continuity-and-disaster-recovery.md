...
description: Learn how this design area can improve business continuity and disaster recovery (BCDR) of the Azure Virtual Desktop.
author: igorpag
ms.author: brblanch
ms.date: 05/18/2021
ms.topic: conceptual
...
With *active-active*: 
* Protects against storage outages without requiring the user to re-authenticate.
* Enables continuous testing of the disaster recovery location. 
* A single host pool can have VMs from multiple regions. In this scenario, usage of [cloud cache](/fslogix/cloud-cache-resiliency-availability-cncpt) is required to actively replicate the user's FSLogix profile and office containers between the regions. 
...
In case of an outage, it's possible to reduce the time taken to back up, restore, and replicate data by: 
...
* In normal usage, the office disk can consume many more gigabytes than the profile disk and the office disk isn't required to be resilient. It is a cache of data and can be downloaded again from Microsoft 365 online services.
