...
- Configure diagnostic settings to [send monitoring data to a storage account](/azure/azure-monitor/platform/resource-logs#send-to-azure-storage) if there's a need to go beyond the two-year retention period. For configuring [diagnostic settings at scale, consider using Azure Policy](/azure/azure-monitor/platform/diagnostic-settings-policy). Additionally, read about [data retention and archive policies in Azure Monitor](/azure/azure-monitor/logs/data-retention-configure) to help you configure retention intervals for session host data.
...
Read more about [creating data collection rules](/azure/azure-monitor/vm/data-collection#create-a-data-collection-rule) in Azure Monitor.
...
- Set up Azure Network Watcher Connection Monitor in the same region as your Azure Virtual Network and session hosts.
