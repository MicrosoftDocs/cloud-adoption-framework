...
- **Require user authentication with Microsoft Entra ID.** Users must authenticate with [Microsoft Entra ID](/entra/fundamentals/what-is-entra) to access desktops and applications from session hosts. Microsoft Entra ID provides centralized cloud identity management for Azure Virtual Desktop.
...
- **Use a non-interactive account for domain join operations.** The domain join account must not require multifactor authentication or interactive prompts. For additional requirements, see [Virtual machine details](/azure/virtual-desktop/deploy-azure-virtual-desktop#virtual-machine-details).
