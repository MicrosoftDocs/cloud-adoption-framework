...
- **Migrate existing virtual desktops to Azure**: A common use case for Azure Virtual Desktop is to modernize an existing virtual desktop environment. Because Azure Virtual Desktop uses a platform-as-a-service (PaaS) control plane that differs architecturally from traditional VDI solutions, there's no direct migration path from other VDI platforms. Instead, your migration will involve:
  ...
  - **Applications**: Deploy applications by using App Attach or install them directly on session host images.
  - **Infrastructure assessment**: Services like [Azure Migrate](/azure/migrate/migrate-services-overview) can help assess your existing infrastructure and dependencies as part of a standard migration process, though the actual Azure Virtual Desktop deployment requires provisioning new host pools.

- **Implement specific Azure products**: Accelerate and improve virtual desktop capabilities by using different Azure products outlined in the Azure Virtual Desktop documentation.
