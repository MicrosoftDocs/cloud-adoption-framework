...
The Azure Purview method is explained in [dataset provisioning by data owners for Azure Storage](/azure/purview/tutorial-data-owner-policies-storage). Note that data owners can also define polices for [resource groups and subscriptions](/azure/purview/how-to-policies-data-owner-resource-group).
