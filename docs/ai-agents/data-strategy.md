...
See [Foundry IQ](/azure/ai-foundry/agents/how-to/tools/knowledge-retrieval?view=foundry&tabs=foundry%2Cpython) and [agentic retrieval](https://learn.microsoft.com/en-us/azure/search/agentic-retrieval-overview?tabs=quickstarts) in Azure AI Search

2. **Plan to use MCP for real-time data and actions**: MCP servers expose functions and queries through a standardized interface. They allow agents to perform live queries against APIs, databases, and SaaS platforms and to take actions such as creating tickets or updating records. MCP ensures secure and auditable interaction with operational systems. In Microsoft Foundry, MCP connects agents to knowledge bases in Foundry IQ, which orchestrate retrieval operations such as query decomposition, parallel execution, semantic reranking, and response synthesis. This integration ensures factual accuracy and transparency through source attribution.
...
4. **Understand trade-offs.** RAG improves speed and reduces API calls but creates stale data for dynamic sources. Live queries ensure freshness but require secure integration and increase latency. Combining RAG and MCP delivers the best experience but adds design complexity. A clear plan prevents confusion and reduces integration risk.
...
1. **Connect to MCP servers.** Where available, use platform-native connections to MCP servers 

2. **Build you own MCP server.**
...
## Plan to connect agents to data
