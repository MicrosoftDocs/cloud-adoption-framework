...
1. **Sensitive data must be isolated from public-facing content**. Agents exposed to external users should only access validated public information, and isolation should be enforced at both the storage and indexing layers. External data sources must be vetted and approved. Platform teams should maintain a list of trusted sources, and workload teams must confirm that each source meets compliance and accuracy standards.

2. **Changes to agent data sources must go through a formal review process.** Changes to agent data sources must be governed through a formal review process. Before integrating any new source, teams must validate its content, intended use, and connection security. Stakeholders should be notified when new sources become part of AI workflows to prevent accidental exposure of sensitive or unapproved data. This process ensures that governance keeps pace with the evolving capabilities and integrations of AI agents.

**Microsoft tools:**

- **Azure AI Foundry:** To implement data security controls for your agents, start by [applying least privilege access principles using Azure role-based access control](/azure/ai-foundry/concepts/rbac-ai-studio) to assign only necessary permissions for specific roles. When your agents need to access Azure services, [use managed identities for authentication](/entra/identity/managed-identities-azure-resources/overview) to eliminate credential management and follow [managed identity best practices for least privilege access](/entra/identity/managed-identities-azure-resources/managed-identity-best-practice-recommendations#follow-the-principle-of-least-privilege-when-granting-access). To isolate sensitive data from public-facing content, [define data boundaries based on user access levels and implement dataset isolation](/azure/cloud-adoption-framework/scenarios/ai/platform/security#secure-ai-data) for different AI applications. Use [Azure Content Safety to filter user prompt inputs and agent responses](/azure/ai-services/content-safety/overview) to prevent harmful outputs and data exposure. For comprehensive oversight, [implement agentic AI safeguards including audit trails, data validation, and independent guardrails](/azure/well-architected/ai/responsible-ai#implement-agentic-ai-safeguards) to maintain data quality and enable detection of anomalous behavior.

- **Microsoft Copilot Studio:** To secure your agents, [configure user authentication with Microsoft Entra ID](/microsoft-copilot-studio/configuration-authentication-azure-ad) rather than allowing unauthenticated access, ensuring only authorized users interact with agents. [Run tools with user credentials by default](/microsoft-copilot-studio/advanced-plugin-actions#authentication-considerations-for-tools) to restrict data access to specific groups or individuals based on their permissions. To monitor agent activity and maintain accountability, [enable audit logging in Microsoft Purview](/microsoft-copilot-studio/admin-logging-copilot-studio) to capture user interactions, administrative actions, and configuration changes. Use [data policies to require user authentication](/microsoft-copilot-studio/dlp-example-3) and prevent unauthorized data access by blocking agents that aren't configured for authentication. To isolate sensitive data, [configure data loss prevention policies](/microsoft-copilot-studio/admin-data-loss-prevention) that govern agent capabilities, knowledge sources, and connector usage based on your security requirements.
