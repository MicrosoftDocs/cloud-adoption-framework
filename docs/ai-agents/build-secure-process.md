...
:::image type="icon" source="./images/foundry-icon.png"::: **Foundry**: Use [prompt-based agents](/azure/ai-foundry/agents/concepts/development-lifecycle?view=foundry&preserve-view=true#types-of-agents) for rapid development. For [hosted agents](/azure/ai-foundry/agents/concepts/hosted-agents?view=foundry&preserve-view=true) to choose your own agent framework on managed infrastructure. See the [AI workloads](/azure/well-architected/ai/) in the Well-Architected Framework. For an enterprise environment, reference and modify the [AI landing zone](https://github.com/Azure/AI-Landing-Zones) deployment. See also [AI Ready](/azure/cloud-adoption-framework/scenarios/ai/ready) for Azure landing zone platform guidance.
...
**Microsoft facilitation:**<br>
:::image type="icon" source="./images/foundry-icon.png"::: **Foundry**: Use [workflows](/azure/ai-foundry/agents/concepts/workflow?view=foundry&preserve-view=true) for visual, low-code design suitable for rapid prototyping.
...
**Standardize agent instructions as configuration to ensure consistent behavior and compliance.** Instructions function as operational guardrails that shape every agent interaction. Treating instructions as code enables version control, peer review, and rollback capabilities when issues arise. Structure instructions to cover: 

| Guideline | Description |
|-----------|-------------|
| **Identity and tone** | Define how the agent presents itself to ensure consistent user experience. |
| **Scope and boundaries** | Explicitly state what the agent does and what it avoids (for example, "Don't access payroll systems"). |
| **Tool integrations** | Mandate specific tool usage. *Example: "You must use the [Tool Name] to answer questions. If the answer isn't found, state 'I don't know'."* |
...
**Validate behavior through rigorous testing.** Test memory to confirm agents don't expose one user's data to another, validate defenses against prompt injection attacks targeting stored context, and verify performance in multi-thread scenarios. Include compliance checks in evaluation cycles to ensure memory usage meets regulatory standards.
