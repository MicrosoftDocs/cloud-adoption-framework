...
**The actions of every agent must be auditable.** Observability enables you to monitor and understand what AI agents do both in real time and over time. Because agents operate probabilistically and don't always behave the same way, visibility into their actions becomes essential for managing risk, ensuring compliance, and optimizing performance. Organizations require complete visibility into agent deployments, behaviors, and costs to maintain control at scale. A control plane keeps the organization in command of every agent. Microsoft Agent 365 provides this when available. When it is not, Entra, Purview, Azure Monitor, and Defender together deliver the same governance fundamentals until Agent 365 can be adopted.

**Recommendation:** Define and operate a single agent control plane that applies across the organization. This plane should provide centralized visibility, consistent identity, enforceable policies, and continuous oversight for all agents. To apply this recommendation, use the following checklist:
...
1. **Assign organizational accountability for agent governance.** AI agents introduce organizational risk similar to applications and identities. Governance requires clear accountability. **Best practices:** Assign ownership for agent governance to the same leaders responsible for cloud governance, security, and compliance. Align agent oversight with existing Azure governance structures. Avoid creating parallel governance models.
    ...
    **Decision guidance:**  If agent usage is limited, existing governance forums may be sufficient. If agents are used across multiple business units, revisit governance and formalize AI agent accountability with defined authority. Central ownership improves consistency. It requires clear decision rights.

2. **Maintain an agent registry** Untracked or "shadow" deployments pose security and cost risks. Organizations discover and classify all AI agents across the cloud environment to maintain a complete inventory of AI assets. You cannot govern agents you don't know exist. **Best practices:** Require every AI agent to be recorded in a single organizational inventory. Track ownership, purpose, platform, and access scope. Treat agents as managed organizational resources. Microsoft Agent 365 provides an [Agent Registry](/microsoft-365/admin/manage/agent-registry?context=%2Fmicrosoft-agent-365%2Fcontext&view=o365-worldwide) when adopted.

    **Decision guidance:** If Microsoft Agent 365 is available, use the built-in registry. If the environment is small, manual tracking may be sufficient for early adoption. If Agent 365 is not available at scale, use Microsoft Entra Agent ID as the authoritative source for agent identities and ownership. This approach provides structure and visibility until a unified registry is adopted.

3. **Require a single identity for every agent.** Agent actions must be attributable and enforceable to a unique identity. **Best practices:** Require each agent to operate under a distinct agent identity. Use [Microsoft Entra Agent ID](/microsoft-agent-365/admin/capabilities-entra?view=o365-worldwide) to assign identity, permissions, and lifecycle controls. Bind agent access to organizational identity policies. Microsoft Entra Agent ID enables consistent identity management across agent platforms and aligns agents with existing Azure identity governance.

4. **Enforce policies consistently across agent platforms.** Agents should follow the same organizational rules regardless of where they run. **Best practices:** Define policies for data access, identity usage, and allowed actions. Apply policies consistently across first-party agents, custom agents, and third-party agents. Do not rely on team-level rules alone. Microsoft Agent 365 supports centralized agent policy configuration where adopted. Use [Agent settings](/microsoft-365/admin/manage/agent-settings?context=%2Fmicrosoft-agent-365%2Fcontext&view=o365-worldwide) to configure allowed agent types, sharing, policies, and user access. On the [Tools page](/microsoft-365/admin/manage/manage-tools-for-agent?context=%2Fmicrosoft-agent-365%2Fcontext&view=o365-worldwide), view AI-powered tools and MCP servers and choose to allow or block them. 

5. **Observe agent activity and tools.** Agent risk changes as behavior and usage evolve. **Best practices:**  Maintain continuous visibility into agent activity, access patterns, and policy compliance. Use observed behavior to identify drift, emerging risk, and gaps in controls. Adjust governance decisions as agent adoption scales. Microsoft Agent 365 provides organizational‑level visibility such a [hero metrics](/microsoft-365/admin/manage/agent-365-overview?view=o365-worldwide&context=%2Fmicrosoft-agent-365%2Fcontext#hero-metrics-for-agent-adoption-and-impact). Use the [Agent Map](/microsoft-365/admin/manage/agent-map?view=o365-worldwide&context=%2Fmicrosoft-agent-365%2Fcontext) to visually see agents. Have teams [auto-instrument their agents](/microsoft-agent-365/developer/observability?tabs=python#auto-instrumentation) to eliminate the need for developers to write monitoring code manually, simplifies setup, and ensures consistent performance tracking.
...
**When Microsoft Agent 365 is not adopted:** If Microsoft Agent 365 is not available, organizations can gather governance signals from separate services such as:
...
2. **Require AI threat protection.** Activate AI-specific threat protection. Microsoft Defender for Cloud's [AI threat protection](/azure/defender-for-cloud/ai-threat-protection) detects prompt manipulation, unauthorized data access, and other agent-specific threats. These protections use global threat intelligence and integrate with Azure AI Content Safety to identify suspicious behavior in real time. Activating these tools reduces exposure and enables faster response to emerging risks.
