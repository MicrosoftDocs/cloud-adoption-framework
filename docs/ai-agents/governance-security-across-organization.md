...
Environments serve as the technical implementation of the governance policies established for AI agents. This phase assumes the organization has selected its platforms as outlined in the [Technology plan](./technology-solutions-plan-strategy.md). The goal is to apply policy guardrails that enforce security controls, access management, and resource limits consistently across all agent workloads while enabling development teams to build rapidly and safely.

1. **Collaborate on enforcement strategy.** Rigid policies often hinder development velocity and innovation. Collaborate with workload teams to determine the optimal enforcement strategy. Start with an audit-based approach to observe behaviors and identify patterns before implementing strict blocking rules. This method allows the organization to refine guardrails without disrupting ongoing development efforts.

2. **Establish the platform foundation.** Define the landing zone strategy based on the service model. For PaaS and IaaS platforms, use management groups to organize workloads and assign governance policies. Secure the environment with network safeguards, such as DDoS protection, and establish network connectivity. This model offers deep customization but requires ongoing platform governance. For SaaS platforms, inherit built-in security controls to reduce operational overhead. Define environment configuration standards that enforce governance policies rather than managing the underlying infrastructure.

3. **Standardize workload infrastructure.** Define infrastructure guardrails in templates that workload teams can deploy. For PaaS and IaaS, use standard application landing zone patterns. AI agents do not require specialized landing zone architecture, but the policies applied differ based on the services used. Provide reference architectures to guide workload teams toward consistent, secure implementations. For SaaS environments, provide templates for common agent patterns to ensure consistency.
