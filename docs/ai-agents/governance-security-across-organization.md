...
:::image type="complex" source="./images/governance-security.svg" alt-text="Diagram showing governance and security framework for AI agents across the organization with four horizontal layers." lightbox="./images/governance-security.svg":::
...
**All agents must meet all baseline security requirements.** AI agents process natural language, interact with external sources, and often make autonomous decisions. These capabilities introduce risks of data leakage where agents unintentionally expose sensitive information, data poisoning where malicious actors manipulate training data to alter agent behavior, jailbreak attempts where attackers bypass safeguards to make agents behave unpredictably, and credential theft where agents become a vector for stealing authentication credentials. Organizations integrate agent security into existing enterprise security frameworks to ensure consistency, reduce exposure, and support regulatory compliance.
...
1. **Integrate with security operations.** AI-related alerts flow into the Security Operations Center (SOC). Define thresholds for anomalies such as latency spikes or unauthorized access attempts. Use Azure Monitor Alerts and route them to Microsoft Sentinel through Log Analytics. This integration strengthens the organization's overall security posture and ensures agents remain resilient and trustworthy.

1. **Audit agent behavior.** Periodic audits of agent interactions identify unusual behavior, such as unexpected data access or different prompt patterns. These audits detect early signs of compromise and support compliance with internal and external standards.
...
1. **Govern external integrations.** AI agents often interact with external systems and data sources. These connections introduce complexity and risk. Teams support interoperability standards such as MCP (widely adopted across platforms) and emerging standards like A2A. They restrict external interactions to trusted MCP servers that meet organizational security and compliance standards. They validate all external agent communications and limit them to explicitly approved entities. They define clear boundaries for data access and logic execution during external interactions. These measures prevent leakage of sensitive information and maintain control over agent behavior across organizational boundaries.
...
:::image type="icon" source="./images/copilot-studio-icon.png"::: **Copilot Studio**: Use the [authoring canvas](/microsoft-copilot-studio/authoring-fundamentals) for low-code development and [custom connectors](/microsoft-copilot-studio/configuration-custom-connectors) to integrate with external data and services.
