...
- **Static knowledge retrieval.** For scenarios where users need reliable answers from a fixed set of information, such as a single knowledge base or a small collection of documents, use a classic RAG approach. The system retrieves relevant content from that source and returns it consistently without combining multiple data streams or performing complex reasoning. You can build classic RAG applications in [Microsoft Foundry](/azure/ai-foundry/concepts/retrieval-augmented-generation?view=foundry&preserve-view=true). This same functionality can also be delivered using declarative agents with single or limited knowledge sources in [Microsoft 365 Copilot](/microsoft-365-copilot/extensibility/overview-declarative-agent) and Microsoft Foundry [prompt-based agents](/azure/ai-foundry/agents/concepts/development-lifecycle?view=foundry#types-of-agents). In these situations, the agent is executing a RAG retrieval workflow, not behaving like a reasoning agent.
