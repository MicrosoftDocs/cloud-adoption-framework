...
Start with a single agent when the task appears simple, stable, and bounded. This approach reduces complexity and cost while supporting fast iteration. However, validate this assumption through prototyping. Do not rely on workflow structure alone—especially if the process is sequential. Instead, test whether a single agent can complete the task end-to-end without confusion, context loss, or role overload. If it cannot, build with a multi-agent architecture.
