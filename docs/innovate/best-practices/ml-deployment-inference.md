...
- **Safe Rollout:** Updates to ML models should be deployed in a phased manner to ensure that the model is performing as expected.  Use the Azure Machine Learning Online Endpoints safe rollout feature to deploy a model to a subset of the traffic and gradually increase the traffic to the new model.  See the [Safe rollout](https://learn.microsoft.com/en-us/azure/machine-learning/how-to-safely-rollout-online-endpoints) article for more details.
