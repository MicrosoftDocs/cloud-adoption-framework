...
- **Batch inference challenges:** While batch inference is a simpler way to use and deploy your model in production, it does present select challenges:
  ...
  - A variation of the cold start problem; there might not be results available for new data. For example, for a retail recommendation system, if a new user creates and account and starts shopping, product recommendations won't be available until after the next batch inference run. If this is an obstacle for your use case, consider real-time inference.
  
  - Multiregional deployment and high availability aren't critical concern in a batch inference scenario. The model doesn't need to be deployed regionally, and the data store might need to be deployed with a high-availability strategy in many locations. This will normally follow the application HA design and strategy.
...
Real-time or interactive inference is the architecture where model inference can be triggered any time and immediate response is expected. This pattern can be used to analyze streaming data, interactive application data etc. This mode allows you to take advantage of your machine learning model in real time and resolves the cold-start problem outlined above in batch inference.

If real-time inference is the right model for running your model there are a number of considerations and best practices to be aware of:
...
**Many models:** The best models for each segment or category are registered in the model registry. It is recommended to give meaningful names to the models that would make it easier to identify for inference.  Where there is a need to group the model into certain categories, it's recommended to use tagging.  
