...
[Azure DevOps](/azure/devops/) includes ways to [request, provide, and manage feedback](/azure/devops/project/feedback/). These tools centralizes feedback so that the team can take action and provide follow-up in service of a transparent feedback loop.

## Continuous integration

Continuous integration is the automating of code multiple times per day to have an updated single project. As adoptions scale and a hypothesis gets closer to true innovation at scale, the number of smaller hypotheses to be tested tends to grow rapidly. For accurate feedback loops and smooth adoption processes, it's important that those hypotheses is integrated and supportive of the primary hypothesis behind the innovation. This requires you to move quickly to innovate and grow, which requires multiple developers for testing variations of the core hypothesis. For later stage development efforts, you might even need multiple teams of developers, each building toward a shared solution. Continuous integration is the first step toward management of all the moving parts.
