...
> [!NOTE]
> It is possible to implement an enterprise-scale landing zone architecture with a single subscription for platform resources, altough, this is only recommended for very small environments such as dev/test or for demonstration purposes. For scalable-production ready scenarios, implement dedicated subscriptions for platform resources as depicted on figures 2 and 3.
