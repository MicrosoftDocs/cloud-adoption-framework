...
> [!NOTE]
> It is possible to implement an enterprise-scale landing zone architecture with a single subscription for platform resources, altough, this is only recommended for very small environments. For scalable, production ready scenarios, implement dedicated subscriptions for platform resources as depicted on figures 2 and 3.
