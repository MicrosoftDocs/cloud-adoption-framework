...
- Use [deny policies](/azure/governance/policy/concepts/effects#deny) to supplement Azure role assignments. Deny policies are used to prevent deploying and configuring resources that don't match defined standards by preventing the request from being sent to the resource provider. The combination of deny policies and Azure assignments ensures the appropriate guardrails are in place to enforce _who_ can deploy and configure resources and _what_ resources they can deploy and configure.
