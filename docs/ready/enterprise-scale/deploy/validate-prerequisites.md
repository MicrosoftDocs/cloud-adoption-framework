...
4. In the sample parameter files (in this case, `10-Create-ManagementGroup.parameters.json`), change the tenant ID and name of the management groups to reflect the tenant where they will be deployed. The tenant ID can be found by running `(Get-AzContext).Tenant`.
