...
[Azure Private Link][link-1] is technology that allows to access a [list][link-2] of Azure services over private endpoints, but it requires that private endpoint records are registered in a corresponding [Private DNS Zone][link-3].

This article describes how application teams can deploy Azure PaaS services in their subscriptions, but only accessible over private endpoints. At the same time, this article will describe how application teams can ensure that their services are automatically integrated with Azure Private DNS Zones, hence there is no need for manual creation or deletion of records in DNS.
