...
A network topology based on Azure Virtual WAN is the preferred enterprise-scale approach for large-scale multiregion deployments where your organization needs to connect your global locations to both Azure and on-premises. A Virtual WAN network topology should also be used whenever your organization intends to use SD-WAN deployments fully integrated with Azure. Virtual WAN is used to meet large-scale interconnectivity requirements. Since it's a Microsoft-managed service, it also reduces overall network complexity and helps to modernize your organization's network.

A traditional Azure network topology should be used if your organization only intends to deploy resources in a few Azure regions, you don't need a global interconnected network, you have a low number of remote or branch locations per region (fewer than 30), or you require full control and granularity for manually configuring your Azure network. This traditional topology will help you build a secure network foundation in Azure.
