...
Platform automation and DevOps evaluates opportunities to modernize your approach to environmental deployment with infrastructure as code options.
...
The scale, agility and flexibility offered with cloud technologies can lead to opportunities for new ways of working and modern approaches to delivering your services.
...
A compound of development (dev) and operations (ops), DevOps is the union of people, processes and technology that continues to provide value.
This approach encourages a team collaboration that can create repeatable processes to help organizations operate efficiently and at scale.
...
The decisions your organization made when reviewing cloud operating models might directly influence how DevOps functions are implemented across technical teams with the capability for infrastructure as code.

### Platform automation

Platform automation focusses on the tools and techniques that enable the streamlining of tasks that might have been performed manually but using automation tools, such as Azure DevOps or GitHub.
...
Using automated processes and tools enables deployment activities to be configured as 'stages' in a pipeline, which is a sequence of tasks that are completed in a prescribed order, with tests and checks built-in to ensure successful completion.

The ability to make changes at scale through a prescribed automated process might provide direct benefits to the organization's ability to expand beyond the baseline configuration, which come from security, governance and management.
...
- Use a policy-driven approach with clear Azure RBAC boundaries to centrally enforce consistency and security across application teams. This ensures good approach is taken by using a combination of Azure RBAC and Azure Policy, and ensures that workloads are compliant with Azure Policy assignments.

- To accelerate Azure adoption, the central platform team might establish a common set of templates and libraries for application teams. For example, horizontal (cross-function) guidance can help support migrations through subject matter expertise and ensures alignment with the overall target enterprise-scale architecture.
...
The Azure landing zone accelerator has the option to integrate continuous integration and continuous deployment (CI/CD) pipelines into the environment using GitHub with GitHub Actions. The portal process guides you through the steps to set up the integration.
