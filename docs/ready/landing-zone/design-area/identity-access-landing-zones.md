...
- Separate administrative access from non-administrative, day-to-day access such as web browsing and e-mail access. Web and e-mail are common attack vectors, and the compromise of a user account is less likely to result in a security breach if the account is not used for administrative access.
...
- Use [administrative units](/entra/identity/role-based-access-control/administrative-units) to provide restricted management of specific objects in your tenant from modification to a specific set of administrators. Administrative units allow for delegated administration of a subset of the directory, such as a service desk that serves only a single business unit within a wider organization. 
  - Use the [Restricted management administrative units](/entra/identity/role-based-access-control/admin-units-restricted-management) feature to further protect specific objects from modification. For example, you can use this feature to prevent the modification of a specific user account, even by a user with the User Administrator role. This can be useful for service accounts that are used by applications, and should not be modified by administrators, or to prevent the modification of a user account or group that has platform or landing zone administration privileges.
...
- When provisioning an application landing zone, the platform team should ensure that all required identity and access management objects are created, such as security groups, standard role assignments, and user-assigned managed identities.
  ...
  - See below for an example of a role assignment structure for the Azure Landing Zones environment. This structure is designed to provide a balance between security and ease of administration. The structure can be adapted to suit your organization's requirements. The same individuals may be members of multiple groups, depending on their role within the organization, but the RBAC assignments should be applied to specific group within a specific landing zone.
...
- Control highly privileged Azure RBAC roles, such as Owner or User Access Administrator assigned to Platform or Application landing zone team members on a subscription or management group, using [Privileged Identity Management (PIM) for Groups](/entra/id-governance/privileged-identity-management/concept-pim-for-groups). With PIM for groups, Azure RBAC roles can be configured to require the same elevation process as Microsoft Entra ID roles.
  - For example, a user may routinely require limited administrative access to resources in an application landing zone. Very occasionally, they may require the *Owner* role. Create two security groups, *Application Administrators* and *Application Owners*. Assign the least-privilege roles to the Application Administrators group, and the *Owner* role to the Application Owners role. Use PIM Groups to allow the user to request the *Owner* role when required. At all other times, the user has only the permissions required to carry out their normal activities.
