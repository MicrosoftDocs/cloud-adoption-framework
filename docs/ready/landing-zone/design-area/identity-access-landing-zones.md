...
- Use [administrative units](/entra/identity/role-based-access-control/administrative-units) to provide restricted management of specific objects in your tenant from modification to a specific set of administrators. Administrative units allow for delegated administration of a subset of the directory, such as a service desk that serves only a single business unit within a wider organization.
  - Use the [Restricted management administrative units](/entra/identity/role-based-access-control/admin-units-restricted-management) feature to further protect specific objects from modification.  Restricted management administrative units can help remove the need for separate Microsoft Entra ID tenants as a security boundary, where separate teams manage the Microsoft 365 platform and the Azure plaform in the same organization. For example, your separation of duty policies may require the use of this feature to prevent the modification of a specific user account, even by a user with the User Administrator role. This can be useful for service accounts that are used by applications, and shouldn't be modified by administrators, or to prevent privilege escalation by modification of a user account or group that has platform or landing zone administration privileges. It can also be used to delegate management of Azure application security principals to the Platform team, without granting privileges on the entire Microsoft Entra ID tenant.
