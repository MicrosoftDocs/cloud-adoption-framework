...
- Consider your JML (joiners, movers, and leavers) process, workflows, and tooling. Ensure that these resouces can support and handle multiple Azure Active Directory tenants.
...
- Consider using the Microsoft identity platform, with Azure Active Directory with B2B and B2C capabilities, to enable developers to create applications in a single Azure subscription, and within a single tenant, to support users from many identity sources. For more information, see [Multi-tenant apps](/azure/active-directory/develop/application-model#multi-tenant-apps) and [Architect multi-tenant solutions on Azure](/azure/architecture/guide/multitenant/overview).
