...
Organizations that operate in the cloud require a directory service to manage user identities and access to resources. Microsoft Entra ID is a cloud-based identity and access management service that provides robust capabilities to manage users and groups. You can use it as a standalone identity solution, or integrate it with a Microsoft Entra Domain Services (Domain Services) infrastructure or an on-premises Active Directory Domain Services (AD DS) infrastructure.

Microsoft Entra ID provides modern, secure identity and access management for Azure and Microsoft 365 services and is suitable for many organizations and workloads. For example, organization with an on-premises AD DS infrastructure and cloud-based workloads might need directory synchronization with Microsoft Entra ID. Directory synchronization ensures on-premises and cloud environments share a consistent set of identities, groups, and roles. Applications that require legacy authentication mechanisms might need Domain Services for managed domain services in the cloud and to extend the on-premises AD DS infrastructure into Azure.
...
- To determine your identity solution requirements, document the authentication provider that each application uses. Use the [identity decision guide](../../../decision-guides/identity/index.md) for help selecting the right services for your organization. For more information, see [Compare Active Directory to Microsoft Entra ID](/entra/fundamentals/compare).
...
- Your hybrid identity strategy in Azure should factor in resiliency as a critical design requirement. Microsoft Entra ID is a [globally redundant, cloud-based system](/entra/architecture/architecture) but Domain Services and AD DS are not. The implementations of Domain Services and AD DS require careful resiliency planning. When designing either service, consider using multi-region deployments to ensure continued service operation in the event of a regional incident.

- To extend an on-premises AD DS instance into Azure, incorporate your Azure regions into your Active Directory [site design](/windows-server/identity/ad-ds/plan/creating-a-site-design).  First, create a site in AD DS Sites & Services for each Azure region where you plan to deploy workloads. Second, create a new subnet object in AD DS Sites & Services for each IP address range you are planning to deploy in the region. Third, associate the new subnet object with the AD DS site you created. This configuration ensures that the DC locator service directs authorization and authentication requests to the nearest AD DS domain controllers within the same Azure region and helps optimize deployment.
