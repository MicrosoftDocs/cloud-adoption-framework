...
-
- Secure the RBAC permissions applying to the domain controller virtual machines. Administrators with RBAC role assignments at the Azure control plane, such as Contributor, Owner, or Virtual Machine Contributor, can run commands on the virtual machines. Ensure that only authorized administrators can access the virtual machines in the Identity subscription, and that overly permissive role assignments are not inherited from higher management groups.
