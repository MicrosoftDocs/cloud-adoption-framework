...
- Your hybrid identity strategy in Azure should factor in resiliency as a critical design requirement. Microsoft Entra ID itself is a globally redundant system (see [What is the Microsoft Entra architecture?](/entra/architecture/architecture)), but for Entra Domain Services and Active Directory Domain Services implementations require careful resiliency planning. When designing either service, consider using multi-region deployments to ensure continued service operation in the event of a region-wide incident.
