...
- [Entra Domain Services](/entra/identity/domain-services/overview) is an Azure-managed service that you can use to create a new managed Active Directory domain hosted in Azure. The domain can have a trust relationship with existing domains and can synchronize identities from Microsoft Entra ID. Administrators don't have direct access to the domain controllers and aren't responsible for patching and other maintenance operations.

- If you're using Entra Domain Services, you can use [replica sets](/entra/identity/domain-services/tutorial-create-replica-set) to extend your managed domain to multiple geographic locations. If you're using AD DS, you are responsible for the placement of domain controller VMs across regions and availability zones.
...
- If you're using [Azure Arc](/azure/azure-arc/overview) to manage on-premises domain controllers or identity services, place the Arc resources in the Identity subscription and apply the same strict access controls as cloud-hosted identity resources.
