...
- Your hybrid identity strategy in Azure should factor in resiliency as an important design consideration. Entra ID is built from the ground up to be a globally redundant system (see [What is the Microsoft Entra architecture?](/entra/architecture/architecture)), but for Entra Domain Services and Active Directory Domain Services implementing resiliency requires careful consideration and planning. When designing either service, please consider using multi-region deployments to ensure continued service operation in the event of a region-wide incident, but more importantly to enable your organization to use Azure as a cloud and consume services with the best performance and availability of services for your organization.
...
- When you deploy Domain Services or integrate on-premises environments into Azure, use locations with [availability zones](/azure/availability-zones/az-overview) for increased availability.  Also consider deploying across multiple Azure regions for increased resiliency.
