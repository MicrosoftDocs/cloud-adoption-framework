...
Landing zone provisioning should allow for both Microsoft Entra ID and Active Directory Domain Services (AD DS) or Microsoft Entra Domain Services to be used by application owners, as required by their workloads. Depending on which identity solution is used, additional functionality may need to be configured as necessary. For example, network connectivity to the Identity VNet should be enabled and secured in cases that require it. If you use a subscription vending process, this should form part of the subscription request.
...
- To access applications that use on-premises authentication remotely through Microsoft Entra ID, use [Microsoft Entra Application Proxy](/entra/identity/app-proxy/application-proxy). Application Proxy provides secure remote access to on-premises web applications. It does not require a VPN or any changes to the network infrastructure. However, since it is deployed as a single instance into Microsoft Entra ID, it will require collaboration between application owners and the platform or identity teams to ensure that the application is configured correctly.
