...
- To extend an on-premises AD DS instance into Azure, create a site in AD DS Sites & Services for each Azure region where you plan to deploy workloads. Assign the appropriate subnets to each of these sites. Properly configuring AD DS sites ensures that the DC locator service directs authorization and authentication requests to the nearest AD DS domain controllers within the same Azure region, optimizing deployment.
