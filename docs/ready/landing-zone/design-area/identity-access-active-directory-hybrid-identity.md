...
Organizations that use AD DS or Domain Services must protect the domain controllers from unauthorized access. Domain controllers are highly vulnerable to attacks and should have strict security controls and segregation from application workloads.
...
- To domain join Windows VMs and other services, you can deploy AD DS domain controllers or Domain Services in Azure. With this approach, AD DS users can sign in to Windows servers, Azure Files shares, and other resources that use Active Directory as an authentication source. You can also use other Active Directory technologies, like group policy, as an authentication source. For more information, see [Common deployment scenarios for Microsoft Entra Domain Services](/entra/identity/domain-services/scenarios).
...
- Your hybrid identity strategy in Azure should factor in resiliency as a critical design requirement. Microsoft Entra ID is a [globally redundant, cloud-based system](/entra/architecture/architecture) but Domain Services and AD DS aren't. The implementations of Domain Services and AD DS require careful resiliency planning. When designing either service, consider using multi-region deployments to ensure continued service operation in the event of a regional incident.

- To extend an on-premises AD DS instance into Azure, incorporate your Azure regions into your Active Directory [site design](/windows-server/identity/ad-ds/plan/creating-a-site-design). Create a site in AD DS Sites & Services for each Azure region where you plan to deploy workloads. Then create a new subnet object in AD DS Sites & Services for each IP address range that you're planning to deploy in the region. Associate the new subnet object with the AD DS site that you created. This configuration ensures that the DC locator service directs authorization and authentication requests to the nearest AD DS domain controllers within the same Azure region and helps optimize deployment.
...
After AD DS or Domain Services is configured, you can use the same method as on-premises computers to domain join Azure VMs and file shares. For more information, see [Compare Microsoft directory-based services](/entra/identity/domain-services/compare-identity-solutions).
