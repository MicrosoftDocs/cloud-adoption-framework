...
- A developer needs a controlled Azure environment to test application design patterns quickly.
...
- A cloud engineer needs a sandbox environment to understand better what happens when a setting is changed on an Azure resource.
...
Place the sandbox subscription in the sandbox management group. For more information about management groups and subscription organization, see [Landing zone design areas and conceptual architecture](/azure/cloud-adoption-framework/ready/landing-zone/design-areas). Azure policies that are created for sandboxes are placed at the management group level of the sandbox. Sandbox environments will then inherit Azure policies from the management group hierarchy that's above it.
