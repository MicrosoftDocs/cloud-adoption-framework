...
- If your organization decides to implement forced tunneling (advertise default routes) to on-premises, we recommend incorporating the following **outbound** NSG rules to deny egress traffic from VNets directly to the internet should the BGP session drop. 
