...
1. **Plan for subscription limits and scalability.** Plan your deployment carefully and ensure that your network architecture is within the [Azure Virtual WAN limits](/azure/azure-resource-manager/management/azure-subscription-service-limits#azure-virtual-wan-limits). If you need more than virtual network connections than a single Virtual WAN limit allows, deploy another Virtual WAN virtual hub. Deploy it in the same region as part of the same Virtual WAN and resource group. 
