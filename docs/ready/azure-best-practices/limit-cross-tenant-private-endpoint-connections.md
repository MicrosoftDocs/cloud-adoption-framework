...
Connections C and D are scenarios that customers generally want to allow as private endpoint connections are kept within the corporate Azure AD tenant and therefore these do not represent a security risk. Hence, these two scenarios are not covered in this article.
