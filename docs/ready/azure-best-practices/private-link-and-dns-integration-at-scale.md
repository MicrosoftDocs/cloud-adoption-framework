...
- If Private DNS Resolver isn't authoritative for customer's corporate domains (for example, Active Directory domain names), it should have outbound endpoint forwards to the customer's corporate domains, pointing to the on-premises DNS Servers (172.16.1.10 and 172.16.1.11) or DNS servers deployed in Azure that are authoritative for such zones.
