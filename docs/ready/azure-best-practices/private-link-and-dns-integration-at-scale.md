...
3. `DeployIfNotExists` policy to automatically create the required DNS record in the central private DNS zone.
    ...
    The following policy examples show two approaches of identifying which `privateDNSZoneGroup` is created on a Private Endpoint.

    The [first policy](#first-deployifnotexists-policy---matching-on-groupid-only) relies on the `groupId` while the [second policy](#second-deployifnotexists-policy---matching-on-groupid--privatelinkserviceid) uses both `privateLinkServiceId` and `groupID`. The [second policy](#second-deployifnotexists-policy---matching-on-groupid--privatelinkserviceid)  should be used when `groupId`'s clash (collide) with another resource. For example, the `groupId` **SQL** is used for both Cosmos DB and Synapse Analytics. If both resource types are deployed and the [first policy](#first-deployifnotexists-policy---matching-on-groupid-only) has been assigned to create the `privateDNSZoneGroup` on the Private Endpoint entry will be created and mapped to the incorrect Private DNS Zone, of either Cosmos DB or Synapse Analytics and then may flap between each of them due to the clashing `groupId` that the [first policy](#first-deployifnotexists-policy---matching-on-groupid-only) looks for in its policy rule.

    For a list of Private-link resources `groupId`, refer to the [subresources column on this page][link-14].

> [!TIP]
> Azure Policy built-in definitions are constantly being added, deleted, and updated. It is highly recommended to use built-in policies vs. managing your own policies (where available). Utilize the [AzPolicyAdvertizer][link-12] to find existing built-in policies that have the following name of 'xxx ... to use private DNS zones'. In addition, Azure Landing Zones (ALZ) has a policy initiative, [Configure Azure PaaS services to use private DNS zones][link-13], containing the built-in policies as well that is periodically updated. If a built-in policy is not available for your situation, consider creating an issue on the `azure-policy` feedback site [Azure Governance · Community][link-15] following the [New built-in Policy Proposals process on the Azure Policy GitHub repo.][link-16]
...
> [!IMPORTANT]
> Below are helpful links to review when creating Private Endpoint with Bicep and HashiCorp Terraform. This article outlines DNS and Private link intigration as scale so there is no need to intigrate DNS with code as the DINE (deployifnotexsist) policy assigned to the Management Group will do it for you.

For Private Endpoint creation with Bicep review [Create a private endpoint using Bicep Quickstart](https://learn.microsoft.com/en-us/azure/private-link/create-private-endpoint-bicep?tabs=CLI). For Private Endpoint creation with HashiCorp Terraform review [azurerm_private_endpoint](https://registry.terraform.io/providers/hashicorp/azurerm/latest/docs/resources/private_endpoint) in Terrafrom Registry. 
