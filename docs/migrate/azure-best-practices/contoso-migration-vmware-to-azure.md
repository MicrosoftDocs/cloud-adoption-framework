...
When fictional company Contoso migrates their VMware virtual machines (VMs) from an on-premises datacenter to Azure, two options are available to them. This article focuses on Azure VMware Solution, which Contoso has determined to be the better migration option.
...
These goals support their decision to use Azure VMware Solution and validate it as the best migration method for Contoso.
...
- Deploy an [Azure VMware Solution private cloud](https://docs.microsoft.com/azure/azure-vmware/concepts-private-clouds-clusters) to the West US Azure region.
- Connect the on-premises datacenter to Azure VMware Solution running in West US by using virtual networks and [ExpressRoute](https://docs.microsoft.com/azure/azure-vmware/concepts-networking) with Global Reach enabled.
...
| **Cons** | <li>Contoso will need to continue supporting the application as VMware VMs rather than move them to a managed service such as Azure App Service and Azure SQL Database. <li>Azure VMware Solution is set up and priced based on a minimum of three large nodes rather than individual VMs in Azure IaaS. Contoso will need to plan their capacity needs, because the company currently uses an on-premises environment that restricts them from the on-demand nature of other services in Azure. |
...
- Configures the HCX components to connect their on-premises vSphere environment to the Azure VMware Solution private cloud.
...
Contoso needs to plan out their networking to include Azure Virtual Network and connectivity between on-premises and Azure. Contoso needs to provide a high-speed connection between the on-premises and Azure-based environments, along with a connection to the Azure VMware Solution private cloud.
...
The Azure VMware Solution private cloud is connected to Contoso's Azure virtual network by using another Azure ExpressRoute connection. ExpressRoute Global Reach will be enabled to allow [direct connection](https://docs.microsoft.com/azure/azure-vmware/concepts-networking#on-premises-interconnectivity) from on-premises VMs to VMs running on the Azure VMware Solution private cloud. The ExpressRoute Premium SKU is required to enable Global Reach.
...
>[!NOTE]
> To learn about network planning for Azure VMware Solution, see [Networking checklist for Azure VMware Solution](https://docs.microsoft.com/azure/azure-vmware/tutorial-network-checklist/).

### Step 2: Create an Azure VMware Solution private cloud

With their network and IP address planning completed, Contoso will next focus on setting up the Azure VMware Solution service in the West US Azure region. With Azure VMware Solution, Contoso can deploy a vSphere cluster in Azure.
...
For more information, see [Azure VMware Solution preview private cloud and cluster concepts](https://docs.microsoft.com/azure/azure-vmware/concepts-private-clouds-clusters).

Azure VMware Solution private clouds are managed through the Azure VMware Solution portal. They have their own vCenter server in its own management domain.

To learn how to create Azure VMware Solution private clouds, see [Deploy an Azure VMware Solution private cloud in Azure](https://docs.microsoft.com/azure/azure-vmware/tutorial-create-private-cloud).
...
1. In the Azure portal, the team creates the Azure VMware Solution private cloud by providing the networking information from their plan and then selects **Review + create**. This step takes about two hours.
...
1. Contoso verifies that the Azure VMware Solution private cloud deployment is complete by going to the resource group and selecting the private cloud resource. The status is displayed as *Succeeded*.
...
An Azure VMware Solution private cloud requires a virtual network. Because Azure VMware Solution doesn't support an on-premises vCenter during preview, Contoso requires additional steps for integration with their on-premises environment. Setting up an ExpressRoute circuit and a virtual network gateway will connect their virtual networks to the Azure VMware Solution private cloud.
...
1. They get the authorization key to connect ExpressRoute to the virtual network. The key is found on the connectivity screen of the Azure VMware Solution private cloud resource in the Azure portal.
...
1. The team connects the ExpressRoute to the VPN gateway that connects the Azure VMware Solution private cloud to their virtual network. They do this by creating a connection in Azure.
...
For more information, see [Learn how to access an Azure VMware Solution private cloud](https://docs.microsoft.com/azure/azure-vmware/tutorial-access-private-cloud).
...
- Contoso is deploying the HCX by using an OVF package that's provided by VMware.
   ...
   To install and configure HCX for your Azure VMware Solution private cloud, see [Install HCX for Azure VMware Solution](https://docs.microsoft.com/azure/azure-vmware/hybrid-cloud-extension-installation).
...
When both the on-premises datacenter (source) and the Azure VMware Solution private cloud (destination) are configured with the VMware cloud and HCX, Contoso can begin migrating their VMs. They can move VMs to and from VMware HCX-enabled datacenters by using multiple migration technologies.

- Contoso's HCX application is online and the status is green. They're now ready to migrate and protect Azure VMware Solution VMs by using HCX.
