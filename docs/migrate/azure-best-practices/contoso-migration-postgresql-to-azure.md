...
- **Automate big data.** Contoso uses PostgreSQL for several of its big data and AI initiatives. The company wants to be able to build scalable repeatable pipelines to automate many of these analytical workloads.
...
| **Azure** | Contoso wants to be able to monitor the system and fire alerts based on performance and security. |
...
After pinning down goals and requirements, Contoso designs and reviews a deployment solution and identifies the migration process. The tools and services it will use for migration are identified.
...
- On the on-premises server, configure the `postgresql.conf` file.
  ...
  - Logical replication must be enabled on the source server by modifying the values in the `postgresql.conf` file for each server.
    - `wal_level` = `logical`
...
- Migration:
  ...
  - In the Azure portal, go to your Database Migration Service resource.
  ...
  - Select **Save**.
  - Select the databases you want to migrate. The schema for each database should have been migrated previously.
  ...
  - Monitor the migration. You might need to retry it if anything fails. An example is if foreign key references were missing.
  - After the `Full load completed` matches your table count, select **Start Cutover**.
  ...
  - Select the **Confirm** check box, and then select **Apply**.
...
- For post-migration, ensure that you've also set up cross-region read replicas, if required, after the migration is finished.
...
- Ensure that its new Azure Database for PostgreSQL instance and databases are secure. For more information, see [Security in Azure Database for PostgreSQL - Single Server}](https://docs.microsoft.com/azure/postgresql/concepts-security).
...
Ensure that the Azure Database for PostgreSQL databases are backed up by using geo-restore. In this way, backups can be used in a paired region if a regional outage occurs.
