...
- Azure paired regions. Each region is paired with another region in the same geopolitical boundary to provide resiliency if a catastrophic region failure occurs. One exception is `Brazil South`, which is paired with `South Central US`. 
