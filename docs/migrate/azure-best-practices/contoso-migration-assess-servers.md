...
Migration of workloads to Azure can be a challenging project and good preparation for migration has significant impact on success and complexity of migration project.
...
Whether you need all the elements described in this article depends on your migration strategy and needs. For example, you might find it's better to discover workload dependencies using existing network monitoring tools or get recommended VM sizes using Movere or other performance monitoring tools.

## Overview

In order for Contoso to prepare plan for migration to Azure, it's critical to plan for the right set of discovery and assessment tools and activities. Deploying and running discovery tools enables Contoso to gather critical information on environment targeted for migration to Azure.

Gathered information will enable Contoso to:

- Build up-to-date inventory of their environments - server and workload estate
...
> - **Step 3: Dependency analysis and Workload discovery.** Which servers are required for specific workload? What other components does workload depend on? What are dependencies between servers and workloads?
...
Based on workflow above and following relevant Azure Migrate documentation, Contoso deploys required appliances and/or agents for discovery. During deployment, Contoso will need to provide server credentials that are used to perform software inventory, agentless dependency analysis, discovery of SQL Server instances and databases. Contoso migration team will collaborate with server and SQL server administration teams to prepare required credentials for discovery. After deployment and configuration, Contoso regularly validates that data is being collected in Azure Migrate project.
...
> [!NOTE]
> Contoso can run multiple assessments. Each assessment can have different properties and use different server groups. All assessments will be stored in Azure Migrate project. This allows Contoso to compare different assessments and decide what migration options are best for their needs and business case (VM sizing, storage type, reserved capacity, etc.).
...
As best practice Contoso reviews server assessment results with workload owners. During review, Contoso will:
...
As a final step in migration planning, Contoso needs to prepare plan for migration waves. Contoso migration team uses all information gathered through assessment, dependency analysis and reviews with workload owners to define migration waves.

Contoso plans for smaller migration waves at the start of migration. With smaller initial waves it is easier to resolve any issues, verify migration process and build confidence in migration strategy. After initial waves, Contoso can plan to include multiple workloads in next migration waves. Typically a migration wave is a single large workload or couple of smaller, independent workloads. However in tightly coupled environments, a migration wave may include multiple workloads that have dependencies.

Contoso uses following [documented guidance](https://docs.microsoft.com/azure/migrate/concepts-migration-planning#prioritize-workloads) to prioritize workloads when assigning them to migration waves. Other best practices that Contoso follows:

- Start with Migration Waves/Groups with least number of interdependent servers.
- Start with Migration Waves/Groups based on lowest business criticality. For example:
...
Contoso plans that some migration waves will change as they progress with migration, therefore migration team reviews and modifies waves periodically. Migration team incorporates learnings from executed waves to improve migration process (such as network requirements, testing/post migration tweaks, permissions, etc.). In case migration wave has to be split and not all servers are migrated as planed in wave, Contoso will:
...
- Schedule the left-out dependencies to be migrated on the next migration wave.
...
Not every step taken here is required for building a migration plan. However as proper migration planning is often key to successful migration project, Contoso invests time in proper migration plan by executing all steps as described above.
