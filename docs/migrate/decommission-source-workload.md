...
1. **Identify unmigrated data subject to regulatory requirements.** Conduct a data inventory of source systems to identify unmigrated data that might be subject to regulatory retention mandates such as HIPAA, SOX, or SEC 17a-4. Work with legal and compliance teams to classify data by regulatory frameworks and document retention periods. Include legacy systems, archived files, and backup repositories that contain regulated data. This assessment prevents compliance violations during source system decommissioning.
