...
- [Select Azure PaaS solutions for AI](./scenarios/ai/platform/resource-selection.md): We simplified language throughout the article, converted workflow descriptions to use numbered lists for better scannability, and refined resource selection steps to be more concise and actionable. We also updated terminology, like changing *Azure Cache for Redis* to *Azure Managed Redis* to reflect current product naming.

- [Secure Azure PaaS for AI](./scenarios/ai/platform/security.md): We simplified language throughout the article by removing passive voice and converting complex sentences to more direct phrasing, which makes security recommendations easier to understand and implement. We also reformatted reference links as a bulleted list and updated the resource table descriptions for better scannability.
...
- [Microsoft Cloud Adoption Framework](.//overview.md): We simplified the introduction by removing excessive detail, consolidated the foundational and operational methodology tables into a single table, and replaced section headings with more direct language. We also removed the *Quick Start* section and reduced the scenarios section to focus on the most relevant options with clearer descriptions.
