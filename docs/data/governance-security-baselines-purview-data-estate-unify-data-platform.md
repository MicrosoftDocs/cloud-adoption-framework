...
2. **Populate the glossary.** With structure in place, create a shared glossary that anchors the organization's most important terms. Terms, such as Customer, Product, Employee, and Location, and measures like Revenue or Headcount, often vary across departments unless harmonized. Document these definitions as [glossary terms](/purview/unified-catalog-glossary-terms) in the Unified Catalog and view them in Purview's glossary. Communicate them broadly to help eliminate ambiguity. This clarity supports consistent AI, analytics, and reporting practices across teams.
...
1. **Classify sensitive data.** [Sensitive data classifiers](/purview/data-classification-overview) in Microsoft Purview identify regulated and business‑critical data so that protection controls can act automatically and consistently across services.

    - **Option 1. Automated pattern-matching.** Use pattern-matching mechanisms, such as sensitive information types, document fingerprinting, and exact data match, when regulatory alignment is the primary goal and the data types match standard definitions.

    - **Option 2. Trainable classifiers.** Add trainable classifiers to the automated pattern-matching capabilities when business‑specific data carries risk that standard pattern-matching doesn't detect, such as trade secrets or proprietary research. Tradeoff: Trainable classifiers increase coverage but require governance oversight to remain accurate over time.
