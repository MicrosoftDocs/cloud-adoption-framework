...
You might need to prepare the target region before relocating the workload. If the relocation adds to your Azure landing zone footprint, you should use the Azure landing zone guidance and then return to this article for workload guidance. For more information, see [How to add regions to Azure landing zones](/azure/cloud-adoption-framework/ready/enterprise-scale/faq#how-do-we-enable-more-azure-regions-when-we-use-azure-landing-zone-architecture). As needed, follow these steps to prepare your workload environment before relocation.
