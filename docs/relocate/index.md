...
Relocation is the process of moving a workload in Azure to a different region in Azure. A workload is a collection of services, applications, and data that support a defined process. It serves as a boundary for resources that you should relocate together. Relocation is a migration effort, but it isn't a migration to the cloud. Instead, it's a migration that stays within the Azure public cloud. Relocation has similarities with the process defined in the [migrate guidance](../migrate/index.md), but it also has its own solutions and considerations to take advantage of different Azure regions. The relocate guidance outlines these solutions and considerations.
