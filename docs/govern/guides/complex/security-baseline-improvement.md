...
**Establishing a corporate IT hub and spoke subscription to centralize the security baseline:** In this best practice, the existing governance capacity is wrapped by a [hub and spoke topology with shared services](/azure/cloud-adoption-framework/ready/azure-best-practices/hub-spoke-network-topology), with a few key additions from the cloud governance team.
...
1. Hub and spoke template:
    1. The guidance in the [hub and spoke topology with shared services](/azure/cloud-adoption-framework/ready/azure-best-practices/hub-spoke-network-topology) reference architecture can be used to generate Resource Manager templates for the assets required in a corporate IT hub.
...
1. Network peering template. This template will peer the virtual network in each subscription with the hub virtual network in the corporate IT subscription.
    1. The reference architecture from the prior section, [hub and spoke topology with shared services](/azure/cloud-adoption-framework/ready/azure-best-practices/hub-spoke-network-topology), generated a Resource Manager template for enabling virtual network peering.
