...
# function Get-StringReplaceAtPosition(
#     [string] $source,
#     [int] $position,
#     [string] $newValue
#     )
# {
#     return $source.Substring(0, $position) + $value + $source.Substring($position + $value.Length)
# }

function Add-ToList(
    [System.Collections.Generic.List[string]] $list,
    [System.Collections.Generic.List[string]] $listToAdd
)
{
    if ($null -ne $listToAdd)
    {
        [void] $list.AddRange($listToAdd)
    }
